...
**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, bridge/play_types.py, config.py, tests/_tmp_engine_run.py, tests/_tmp_fly_check.py（新增）, tests/_tmp_probe_window.py（新增）

**测试验证**: 滑动窗口单测（无流程 AKQ 窗口/飞牌花色 K 出后滑到 J/非飞牌花色 Q 出不补位）PASS；`_tmp_fly_check`（♠(T) 排除、♦(J) 保留）PASS；seed42 6NT 回归 12 墩做成、无假 ♥T 过手/♠T 启动；后端重启后 8003 健康检查通过
