...
**背景**:
LLM 打牌有两个固有问题：(1) API 延迟高（5-30s 每次出牌）；(2) 无法做全局前瞻搜索（每墩独立决策，缺乏对后续墩的推演）。决定引入 ISMCTS（Information Set MCTS）作为替代引擎，与 LLM 引擎共存可切换。

**技术方案**:

1. **ISMCTS 架构** (`bridge/mcts/` — 7 个新文件):
   - `state_utils.py` — 手牌克隆、合法出牌计算、状态推进
   - `constraints.py` — `BidConstraint` 数据类 + `validate_sample()` 叫牌约束验证
   - `sampler.py` — `DealSampler`：从当前玩家视角采样完整手牌分布，支持叫牌约束过滤 + 加权 biased 采样
   - `rollout.py` — `HeuristicRollout`（确定性启发式）+ `RandomizedRollout`（20%随机 + 长四首攻/第二家小/第三家大）
   - `search.py` — `MctsSearch`：Determinization + UCT 搜索，自适应迭代（牌张越少搜索越快）
   - `constraint_extractor.py` — LLM 从叫牌历史提取 BID_CONSTRAINT 提示词
   - `__init__.py` — 包导出

2. **MCTS 搜索流程**（每次出牌决策）:
   ```
   for iteration in 1..adaptive_cap:
       1. Determinization: 从未知牌中采样完整手牌 ← DealSampler
       2. Selection + Expansion: 沿 UCT 走树，到达叶节点后扩展一个子节点
       3. Simulation: RandomizedRollout 快速模拟到底 (20% 随机)
       4. Backpropagation: 回溯更新所有祖先的 visits 和 value
   选择 root 下 visits 最多的子节点 → 最优出牌
   ```

3. **叫牌约束采样**:
   - 每局开始时 LLM 从叫牌历史提取约束（HCP 范围、花色最少张数、均型/非均型）
   - 采样时用加权 biased 算法倾向满足约束的牌，再经 validate_sample 验证
   - 约束缓存：一局只提取一次

4. **自适应迭代**:
   ```
   remaining = 52 - 已知牌张数 - 已出牌张数
   iterations = MCTS_MIN_ITERATIONS + (MCTS_ITERATIONS - MCTS_MIN_ITERATIONS) * (remaining / 52)
   ```
   开局满手 → ~3875 次。残局 10 张 → ~1100 次。

5. **引擎切换**:
   - `config.py` → `DEFAULT_PLAY_ENGINE = "llm"` / `"mcts"`
   - 前端打牌设置面板可选引擎
   - API 响应 `used_engine` 字段区分渲染逻辑

6. **前端 MCTS 可视化** (`PlayDetailPanel.jsx`):
   - 当 `used_engine === "mcts"` 时渲染水平柱状图
   - 每行：牌张 | 柱状条（宽度 ∝ visits/maxVisits）| `{visits}次 · {avg_tricks}墩`
   - 顶部摘要：`{iterations}次搜索 · {time_sec}s · {iters_per_sec}it/s`
   - 纯 MUI Box 实现，深色/浅色主题适配

**配置** (`config.py`):
```python
MCTS_ITERATIONS = 5000          # 最大迭代数
MCTS_TIME_LIMIT = 10.0          # 时间上限 (秒)
MCTS_EXPLORATION_CONSTANT = 1.414  # UCT exploration 参数
MCTS_MIN_ITERATIONS = 500       # 自适应下限
ROLLOUT_GREEDY_PROB = 0.80      # Rollout 启发式概率 (20% 随机)
DEFAULT_PLAY_ENGINE = "mcts"    # 默认打牌引擎
```

**性能**:
- Benchmark: 1920 it/s（纯计算）
- 实时搜索: 500-900 it/s（含约束采样开销）
- 首墩 ~3875 次迭代 / 4-7s，末墩 ~500 次 / <1s

**修改文件** (新增):
- `bridge/mcts/__init__.py`
- `bridge/mcts/state_utils.py`
- `bridge/mcts/constraints.py`
- `bridge/mcts/sampler.py`
- `bridge/mcts/rollout.py`
- `bridge/mcts/search.py`
- `bridge/mcts/constraint_extractor.py` (BID_CONSTRAINT_PROMPT 占位)

**修改文件** (已有):
- `bridge/play_service.py` — 新增 `_mcts_play()`、`_get_bid_constraints()`、`get_ai_play(use_mcts=)` 分支
- `bridge/play_types.py` — `PlayState` 增加 `get_playable_cards()`、`bidding_sequence` 字段
- `api/main.py` — `PlayAIRequest.play_engine`、`PlayAIResponse.used_engine` 字段
- `config.py` — 6 个 MCTS 配置项
- `web/src/App.jsx` — 打牌引擎选择 + MCTS 记录传递
- `web/src/components/PlayDetailPanel.jsx` — MCTS 柱状图 + 引擎标识
- `web/src/services/api.js` — `aiPlay()` 增加 `play_engine` 参数

**技术要点**:
- MCTS 同步阻塞 → `asyncio.to_thread` 放入线程池避免阻塞事件循环
- 约束采样 rejection loop 优化：从纯随机+拒绝改为加权 biased + 10 次重试安全网
- `RandomizedRollout` 每步 20% 概率随机出牌，80% 走启发式（长四首攻、第二家小、第三家大）
