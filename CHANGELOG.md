...
## 2026-09-27（探分布先兑现 v2.17 + 采样交集预分配 + 窗口期显示文案）

**背景**: 3NT 牌局（南 ♠A2♥AK32♦A32♣QJ32 / 北 ♠43♥4♦QT876♣AT987）墩2 引擎 ♣Q 与 ♦A **并列 54.7%**，飞牌介入因 ♣ 有直接飞 K 结构启动 ♣Q——但"先拔 ♦A 探分布：敲下单张 ♦K 则转 ♦ 线、没敲下再飞 ♣ 不迟"的机会被错过。介入层只识别"本墩**直接飞**"结构，不产生"先兑现探分布、结果驱动转线"的延迟动作。

**口径（用户定调 2026-09-26/27）**:
1. **探分布先兑现分支**（一般情形门控之后）：动作/榜首 ≥ FINESSE_NEC_RATIO (0.85) 通过后，先查 `_safe_cash_action`——**引擎榜首**（顶张）是**非飞牌花色** A（NT 稳拿不丢权）或 K 且队友持同花色 A（同花色 A 唯一：队友持 A ⇒ 敌方无 A ⇒ 出 K 无人能压 ⇒ 稳赢保权）→ 先出 A/K 兑现，**不登记飞牌流程**（未启动），下墩自然重新探测验证——飞牌结构仍在、门控仍会过，机会不丢；兑现若敲下关键张，下墩局面已变，重新验证自然走新线。
2. **顶张限制**（用户修正）：仅检查 `candidates[0]`（引擎顶张）——非顶张的 A/K 不得用于探分布（引擎不认为出它最优时强行兑现会违背引擎判断）。
3. **契约必要分支不受影响**（榜首<0.50 时必飞，不探分布）。

**改进**:
- `play_service._safe_cash_action` + `_probe_lead_finesse_prefer` 过闸接入；契约必要分支不查兑现
- `sampler._sample_uniform` **lead_shape 交集预分配**：剔首攻牌后各形态大牌集交集（长套攻K → {Q}）定向放入该位置 + 小牌垫底至 max(smin)；交集为空（KQJ+/KQ+/Kx/K 混合）→ 不预分配保 Kx/K 多样性。此前 L0 MH 300 步凑"Q+恰2小"失败落放宽链丢约束（实测 6% 无 Q、20% 长度<3 污染 DD 决策）→ 修复后 100% 含 Q、100% 长度≥3
- **窗口期显示文案**（用户反馈"完全不懂"）：说明直接为用户格式 `飞K(单飞)押100%直出♣Q`（伙伴侧"押X%过手♣2给队友引飞"），"押桶成排序·第N候选"移入后端日志；`改出/维持` 按改选与原选是否相同区分；前端 `窗口期启动` 直接显示说明不再拼重复字段
- 桥牌逻辑注释修正：K+队友A 的保权依据为"A 唯一 ⇒ 敌方无 A"（此前误写"敌方 A 压则队友盖吃"——同花色 A 只有一张，前提不成立）

**验证**: pipeline **33/33**（+t42 榜首=其他花色A 先兑现不登记 flow；+t43 榜首非A 时候选里非顶张 A 不触发 → 正常启动）+ probe 12/12 + lead_shape **76/76**（新增出 K 后剩余口径 Q2 命中/A2 拒绝/Kx 分支）+ doc-sync ✓；后端已重启

**修改文件**: bridge/play_service.py, bridge/mcts/sampler.py, web/src/components/play/shared.jsx, tests/test_finesse_pipeline.py, docs/飞牌介入管线图解.md, CHANGELOG.md, DEVELOPMENT.md
...
**用户定调**: "清将线做成率 ≥ 基准做成率即可触发"，段2 触发判据由逐世界墩差改为**成约率口径**。

**改进**（bridge/play_service.py `_clear_trump_lead`）:
- 保留 门B（将牌探针 Δ 未达标才本层出手）、门A（top-N 做成率趋同极差 <0.03）、去飞角世界剔除（剔除后不足一半交段4）不变
- **判据替换**：保留世界上 `make_clear = 清将线拿墩≥需求墩的世界占比`，`make_base = 基准线（引擎最优不清将候选）同口径占比`；`make_clear ≥ make_base` 即无损触发改出最小将牌；`make_clear < make_base` → 交还引擎/段3/段4
- `无损清将` 注记字段由 `"逐世界"`（保留/总数/最小差）改为 `"成约率"`（清将/基准/保留/总数），提示文案同步
- 删冗余探针脚本 tests/_dbg_c7_probe.py；保留四变体复现 tests/_dbg_c7_cleartrump.py

**修改文件**: bridge/play_service.py, docs/飞牌介入管线图解.md（补 §二-A 判据小节）, CHANGELOG.md, DEVELOPMENT.md

**测试验证**: verify_clear_trump_4s 4/4（CASE1 触发文案含"清将不减做成率(100.0%≥99.0%)"）+ doc_sync 通过 + test_finesse_pipeline 40/40 + test_probe_finesse 8/8；C7 live 精确复现：♠3 做成率 ≥ ♦A → 触发清将（旧判据 45.9% 负世界必拒）

## 2026-09-22（DD 介入层五段顺序：无损清将 v2.06）

**背景**: 用户定调"无损失先清将原则":此前 9砸与飞牌为并列双分支,9砸 若动将牌(对方合计仅4张)极可能被将吃,且清将本身也有要飞的问题,须放在清将检测之后;无损清将须叠加做成率趋同门与飞牌探针 Δ 不达标门;NT 自然退化跳过。逐世界比较复用 DD 候选 per-world scores + last_worlds 共享采样,替代不利分布桶(§十一已证 DD 全知桶失敏)。

**改进**:
- **开关改名**:`DD_FINESE_ENABLE` → `DD_INTERVENE_ENABLE`(介入总开关,统一管 清将/9砸/飞牌;关闭则 DD 仅按引擎得分选牌,αμ 不受影响)
- **`_intervene` 重构为五段顺序**(先命中先赢):段1 稳成(`_top1_make`≥**1.00**,0.85→1.00,只板上钉钉才闭嘴,85~100% 交精修)→段2 无损清将(`_clear_trump_lead`,仅**有将定约**,NT 退化跳过)→段3 9砸(`_garrison_*`)→段4 飞牌(`_finesse_*`)→段5 多数投票(`_dd_play` 内兜底)
- **新增 `_clear_trump_lead`**:门A top-N 做成率趋同(极差<`_CLEAR_TRUMP_EPS`=0.03)+门B 将牌飞牌探针 Δ<`FINESSE_PROBE_DELTA`(飞角型交段4);判据=逐世界 `min将牌赢墩 − 最优不清将候选赢墩`,剔除"清将依赖飞角牌"(敌持缺飞对象 K)的世界,剩余全 diff≥0 且不足过半才选最小将牌
- **`FINESSE_NEC_MAKE_HIGH` 0.85→1.00**:段1 收敛为引擎 top1 做成率口径,4♠ top1=99.8%<100% 不被挡、能进无损清将

**修改文件**: bridge/play_service.py, config.py, api/main.py, docs/飞牌介入管线图解.md, tests/test_finesse_doc_sync.py, tests/verify_clear_trump_4s.py(新增), CHANGELOG.md

**测试验证**: verify_clear_trump_4s 4/4 PASS(4♠ 触发清将♠3;100%稳成/diff<0/探针Δ达标三护栏正确退让);test_finesse_doc_sync 通过;test_finesse_pipeline 40/40 + test_probe_finesse 8/8

## 2026-09-22（飞牌对象窗口 3→2：AK 起步 v2.05）

**背景**: 用户指出基本不存在三飞，窗口第三对象白耗探针分桶计算。窗口 3→2 从 [A,K] 起步，省约 1/3 探针分桶。（首次实现的依据" A 永远判 False"是错误理解——`obj > G > max_enemy` 只要求 G 小于 obj，A 是有效对象；该依据已纠正。）

**改进**（bridge/mcts/dd_search.py `_honor_missing_of_state`）:
- **`cands[:3]` → `cands[:2]`**：窗口 2 张滑动、AK 起步、出一个向下补一个（下限 8 不变；基座 `_FINESSE_WINDOW_BASE` 不变）
- **依据（2026-09-22 用户纠正后）**：A/K/Q 同时为对象的三飞局面在桥牌中基本不存在——A 缺时通常是 K 对 A 飞（**对象=A、G=K**：A=14 > K=13 > 防家除 A 外最大牌，注意 `obj > G > max_enemy` 只要求 G 小于 obj，A 是有效对象绝非占位）；窗口 2 张覆盖"对象+间张"所需的前两个缺失大牌，省约 1/3 探针分桶计算
- **注意**：窗口必须从 A 起步、绝不能从 K 起步跳过 A（A 缺失时是飞牌对象）
- 滑动语义不变：`state.finesse_windows[suit]` 持久、各花色独立

**修改文件**: bridge/mcts/dd_search.py, docs/飞牌介入管线图解.md, CHANGELOG.md

**测试验证**: test_finesse_pipeline 40/40 + test_probe_finesse 8/8 + doc_sync 通过（窗口非符号/阈值，doc_sync 不受影响）

## 2026-09-21（飞牌领出侧执行顺序修正：先结构确认后稳成检测 v2.03b）

**背景**: 用户重打牌局发现"出♦A还报领出飞牌·对象T"——稳成检测在结构确认**之前**执行，且用 `_detect_finesse_struct` 的**未确认**探针（仅 dd_search 探针层 Δ 达标，未过 `_probe_finesse_ok`）判断"引擎 top1 是否在飞牌花色"。商品牌（防家仍有更大牌，如对象T 对防家还有 ♦Q）下该探针 G 判定实为 False，却被借用于登记 `finesse_flow`，UI 报出假对象。用户定调：**未确认探针不能作为"top1 是否在飞牌花色"的判断依据，先确认后稳成**。

**改进**（bridge/play_service.py `_finesse_lead`）:
- **执行顺序重排**：结构探测（本侧+伙伴侧）→ `_probe_finesse_ok` 逐条确认 → 建确认结构池 `struct_stack` → **稳成检测**（`_top1_make` ≥0.85） → 未稳成才走榜单决策
- **稳成与已确认池对比**：引擎领出花色在确认池中 → 仅登记本墩接应（说明标注"结构已确认"）；不在池 → 提前退让。未确认探针不再进入登记路径
- **移除旧"稳成前置"**：原 line 1813-1840 入口即判稳成、用未确认 `local` 判登记的分支删除，合并入确认后的稳成检测
- t20 原期望（南♣A2/北♣43 报对象K → 登记）本身即用未确认探针——该牌面 `_probe_finesse_ok` 判 False（K>G>max_enemy 无解），改为真飞结构（南♣A3/北♣Q2，防家 K+J，G=北Q=12 满足 13>12>11）验证"稳成 + 引擎在确认池 → 登记"

**修改文件**: bridge/play_service.py, tests/test_finesse_pipeline.py, docs/飞牌介入管线图解.md, CHANGELOG.md

**测试验证**: test_finesse_pipeline 40/40（t20 改为已确认结构路径）+ test_probe_finesse 8/8 + doc_sync 通过；后端 8003/前端 5173 重启健康
...
**改进**:
- **v2.00 启动简化**（用户定调）：`_probe_lead_finesse_prefer` 删路线对决/逐动作门控循环/模板法兜底 → 过手预检后全局押桶成排序保留榜首，榜首单独过门控（判据不变），通过即登记终选。
- **v2.02 押注桶统一口径**（"严峻"名废弃，用户定调"严峻桶和押注桶应该一致"）：押注方向=几何常数（引牌侧=领出者下家 / 接应侧=接应者上家，南领出北接应时同为西）。押注桶=被飞对象**全部命中押注方向**的世界做成率：单飞=对象在下家；双飞=登记+废弃两对象都在押注方向。所有决胜（门控分子/终选排序/接应判据）以押注桶成为据。
- **v2.03 终选较大领出牌决胜**（本次，用户定调）：`_subset_select` 排序键 `(a_make, blended)` → `(a_make, big, blended, rankpos)`。押桶成平票之后，偏好"领出牌 牌点 > min(对象, *废弃对象)"的较大牌——**避开出小飞牌被较小/第二飞牌对象"白吃"**；不区分单双飞（单飞仅一对象=min对象、领出牌必小于对象→判据失效；双飞 K/9 → min=9，♦J 即大牌）。`big` 之后仍回引擎 blended 决胜，`rankpos` 兜底。

**测试验证**: 新增 t40（组合双飞全中饱和平票 → 改出 ♦J）；回归 test_finesse_pipeline 40/40 + 探针 8/8 + doc_sync 通过。图解 §三 F / 符号表 / 分解 §三.1 同步；项目记忆已记。

---

## 2026-09-21（飞牌启动/终选押桶成口径贯通 v1.97→v1.99+：双飞强信号合并 · 接应严峻子桶 · 终选押桶成主排序 · 早退分支收紧）

> 本节记录**截至 2026-09-21 安全检查点提交（49d37df）**的已落地改动。v2.00 简化与领出端严峻桶（v2.01）**尚在探讨中，暂不记录**（代码在工作区，定稿后再补）。

**背景**: 重打 6NT 实局发现——北未接应出 ♦J（出 ♦A）等问题反复，根因是同一花色多候选引牌的裁决口径长期混用：探针 Δ（位置敏感度）、引擎值（全样本混合）、押桶成（押注半桶）在不同环节各自为政，导致同对象多条达标引牌时而选小、时而选大、接应端强制飞张被比值误杀。

**改进**:
- **v1.97 双飞合并·强信号门**（用户定调"按加和方案实施"）：`_finalize_finesse_probe` 双飞门扩触发——原 v1.90 弱信号门（单条 Δ 均 < 阈值才合并）在做成率口径下很少触发，双缺 KQ 以两条独立单飞出现、`_probe_finesse_ok` 单对象威胁判定互压必判废。新分支：同花色 ≥2 **达标**对象同样合并——保大对象（K）、小对象记「废弃对象」（威胁计算排除）、Δ 取加和（总敏感、展示排序压过普通单飞）、组合飞=True；G 判定不过仍照常判废
- **v1.98 接应严峻子桶 + top_alt 排除等价组**（用户定调"按这个方案实施"）：①分桶端按「登记对象方向 × 废弃对象同/异侧」细化桶键（`东·严峻`/`西·严峻`），消费端三层判据优先读严峻子桶（同侧世界=双威胁存活、出A必宕的真实风险），剔除"废弃对象异侧被顺吃"的假收益——出A两桶虚高的主因是 Q 干扰而非双明手偏差；②`_finesse_commit_ratio_ok` 的 top_alt 跳过与强制牌逐世界 scores 全等的等价组候选（接应小牌组=同一动作，拿它算 b_bucket 是评估自己），取真正大牌
- **v1.99 终选押桶成主排序**（用户定调"按这个方案改"）：`_subset_select` 对过闸飞牌动作按**押桶成**主排序、引擎决策值（make_rate 下做成率×10000+平均赢墩）决胜；押桶成缺失回退引擎全样本做成率。旧口径复用引擎判据会让非押注世界稀释排序（本局实案：混合值小牌组 48.0% > 大牌组 47.3% 选中 ♦2，混合值含"K 异侧飞必败"世界）
- **v1.99+ 早退分支收紧**（用户定调"即使引擎选牌已在飞牌花色也要用押桶成决胜"）：引擎 top1 恰为本侧引牌时不再无条件尊重——该花色全部本侧未现身条目经 `_subset_select` 押桶成终选，胜者登记、可改出；同 (花色,对象,侧) 多引牌取档代表改押桶成分档（0.02 粒度、同档 Δ 决胜，模板法回退引擎值档）

**双飞合并口径确认（押桶成不随 Δ 加和）**: 合并条目押桶成 = 保 K 引牌押注半桶做成率，**不随 Δ 加和**；Δ 加和仅用于合并触发门票与展示排序，不参与启动价值排序。启动选牌自 v1.99 起按押桶成，双飞与普通单飞同秤（v1.97 注释"排序压过普通单飞"指展示性 Δ 排序）。

**测试验证**: t30（强信号合并+Δ加和+单对象不合并对照）/t31（合并确认、K 出后续飞 Q）/t32-t34（接应严峻子桶三态）/t35（押桶成翻转排序）/t36（早退分支收紧）；回归 36/36 + 探针 8/8 + doc_sync 通过。图解 §三 B/C/F、§六、§八 同步；项目记忆已记。

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, tests/test_finesse_pipeline.py（t30-t36 新增/改写）, docs/飞牌介入管线图解.md, tests/test_finesse_doc_sync.py, config.py（FINESSE_COMMIT_DIE_PCT/ALIVE_PCT 新增）
...
**改进**:
- **数值型随请求携带**（`PlayAIRequest` + `_execute_ai_play`）：`dd_majority_votes`（v1.90 已做）、`alpha_mu_particles`、`alpha_mu_m` 每决策经由 ai-play 请求带入并即时应用（免刷新/免同步/免时序）；`_alpha_mu_play` 拆 wrapper+inner 支持请求级临时覆盖（与 `_dd_play.dd_samples` 同模式，finally 恢复）；前端 api.js `aiPlay` 增参 + App.jsx 传递。计分制 `dd_scoring_mode` 与样本数 `dd_sample_count` 本就是请求携带（确认无此病）
- **全局开关落盘**（api/main.py）：`runtime_config.json`（项目根，已加 .gitignore）持久化 6 项可覆写开关（DD_FINESSE_ENABLE / DD_USE_CONSTRAINTS / DD_KEEP_SURE_WIN / DD_KEEP_CRITICAL / DD_KEEP_SURE_LOSE / FINESSE_PROBE_DELTA）；服务启动模块加载时 `_load_runtime_overrides` 恢复，4 个 set 端点改动后 `_save_runtime_overrides` 写盘
- 前端启动同步 useEffect 移除 `dd_majority_votes` 推送（改由请求携带，避免覆盖面）

**测试验证**: 22/22 + 8/8 全过；持久化闭环实测——改 delta=0.45 → 重启后端 → GET 仍 0.45（`[runtime_config] 已恢复 6 项`）→ 还原 0.4；票数经请求携带由用户实测触发（`[DD多数投票] 5票 ♣A:3/♣2:2`）；前后端健康

**修改文件**: api/main.py, bridge/play_service.py, web/src/services/api.js, web/src/App.jsx, web/src/hooks/useModelSettings.js, .gitignore, CHANGELOG.md, DEVELOPMENT.md

## 2026-09-19（组合飞探测门收紧为双飞：Δ 最大两对象加和 v1.90）

**背景**: 用户实测当前牌（南♠ 长套）探针误检出"♠A,Δ0.44"假飞牌结构，介入改出♠T。日志显示三个对象 A(0.12)/K(0.16)/Q(0.16) 全未达标，旧"组合飞门"把**全部**对象 Δ 加和（0.44≥0.4）并保 rank 最高的 A（对象 A 的 _probe_finesse_ok 又因废弃 K/Q 从防家牌中剔除而算法性通过）——三信号噪声叠加出假结构。用户定调：**桥牌不存在同花色三飞，组合飞语义就是双飞**。

**改进**（dd_search.py `_finalize_finesse_probe`）:
- 组合飞门 → **双飞门**：只取 Δ 最大的两个对象加和 ≥ FINESSE_PROBE_DELTA 才达标（KQ 分家/同家加总显著才是双飞真实敏感；多个弱信号叠加≠真双墩敏感）
- 保 rank 较高者（AQ 双飞保 A、KQ 保 K）——**对象 A 不排除**（缺 AQ 持 KJ 引小是标准双飞 AQ 结构）
- 日志 `[组合飞探测]` → `[双飞探测]`；`组合飞`/`废弃对象` 字段名保留供下游兼容

**测试验证**: t01-t22 全过、test_probe_finesse 8/8；后端 8003 重启健康。本例回归：A/K/Q 三对象按 Δ 最大两个加和 = 0.16+0.16=0.32 <0.4 → 无 ♠ 结构，不再介入

## 2026-09-19（飞牌介入选牌流程重构：稳成前置 + 留栈回退 + 逐动作门控 + 终选复用引擎 v1.89）

**背景**: 梳理飞牌介入选牌流程时确认三个缺陷：① 每花色按 Δ 剪枝为单条，丢弃"Δ 小但动作价值高"的结构（Δ 只代表位置敏感性，不该当选牌判据）；② 伙伴侧过手预检失败 → 整花色出局无回退（本侧 Δ 小≠不能飞）；③ 门控 first-fit——第一个过闸花色即启动，后面更好的不参与。方案定稿于讨论（用户逐条定调）。

**改进**（全在 bridge/play_service.py）:
- **改动C·稳成前置**（`_finesse_lead` 入口）：全体候选最高做成率 ≥95%（`FINESSE_NEC_MAKE_HIGH`）→ 直接退让，不做探测/过手预检（省一次完整伙伴侧 DD search）；仅"引擎 top1 恰在本侧飞牌花色"时登记本墩接应（稳成不影响流程内接应）
- **改动A·留栈**（by_key 后）：同一花色全部代表条目保留（Δ 降序），不再剪枝为单条——Δ 只当"进池门票"
- **改动B·过手预检逐条**（`_probe_lead_finesse_prefer`）：伙伴侧条目逐个求安全过手牌，失败只删该条目（备胎顶上，不再整花色消失）；动作统一为真实出牌（本侧=引牌 / 伙伴侧=过手牌）
- **门控重构**（`_finesse_launch_worthwhile` 改逐动作判据）：分子从"花色最优候选"（顶张虚高）改为"动作牌价值"；稳成分支删除（已前置短路）。契约必要（榜首做成率<50% 且动作/榜首≥50%）必飞，否则比值≥70% 启动
- **终选=`_subset_select`**：过闸动作作为候选子集，完全复用引擎选牌判据（`dd_search._compare_candidates`，make_rate 模式做成率主 + avg_tricks 决胜）选最优 + 登记 flow——票选机制预留（`DD_MAJORITY_VOTES`=1 默认关）
- **⑤ 引擎一致收紧**：仅"top1 == 本侧引牌（正在飞）"时尊重+登记；top1 同花色非引牌（顶张兑现）不再整体退让，交给门控裁决
- **删除"直接飞小牌"分支**：探针法动作必在候选池；模板法（αμ）条目由动作池兜底生成最小飞张
- 组合飞维持现状：废弃对象字段随胜出条目透传登记（验证 `flow={'♠':11}+废弃[T,9]`）

**修改文件**: bridge/play_service.py, tests/test_finesse_pipeline.py, CHANGELOG.md, DEVELOPMENT.md

**测试验证**: 新增 t16（过手失败弹栈回退本侧）t17（全被否尊重引擎）t18（多花色终选引擎判据）t19（稳成提前退让不登记）t20（稳成但引擎在飞牌花色仍登记）t21（Δ 不进选牌）t22（票选默认关），22/22 全过；`test_probe_finesse.py` 回归 8/8；临时冒烟（真实 DDSearch 4 花色结构共存 + 组合飞登记）通过；后端 8003 重启健康 200
...
**改进**:
- **首攻格式**：`opening_lead` 由 `f"{POS}:{rank}{suit}"` 改为 `f"{POS}:{suit}{rank}"`（如 `西:♥K`）
- **手牌排序**：`sorted(..., key=lambda x: -RK.index(x))`（实际输出从小到大）改为 `sorted(..., key=RK.index)`（A→2 从大到小）

**修改文件**: tools/bm_import.py, bm_deals.json（重生成）, CHANGELOG.md, DEVELOPMENT.md

**测试验证**: 537 副全量校验——手牌非降序 0 副（南 ♠AKJT9 ♥32 ♦KQJT9 ♣A）、首攻格式异常 0 副（样例 `西:♠T`/`西:♥J`/`西:♠K`）、0 渲染失败、HCP 全对；后端 API 实时读文件确认 `西:♥K`，前端纯字符串展示无解析依赖
...
- **9砸 独立分支 `_garrison_lead`/`_garrison_follow`/`_garrison_target`**：领出侧 = 连拔检查（cash_bank 私有跨墩状态，不受稳成线约束）→ 稳成线退让 → 四门按联手张数降序扫描 → A 在领出方手直接砸 A（AK缺Q 登记 cash_bank 下墩连拔 K）；A 在伙伴手引最小小牌+九砸标记（伙伴超吃）。跟牌侧 = 引擎决策牌为低于对象的间张（10≤rv<obj）时改出 A。判据仅两种情况（2026-09-18 用户定调直白化）：**AK缺Q（Q 未现）砸 A 后连拔 K / AQ缺K（K 未现）砸 A**；其他（如 AKQ 在手顶张齐全）一律不走 9砸，交回引擎自然兑现。实测验证探针在满手大牌场景 Δ 互偿趋零（位置不敏感），9砸 判据不需要位置信息故独立分支零漏检
...
**测试验证**: 新建 `tests/test_finesse_pipeline.py` 13 用例全过（9砸扫描命中/AQ场景/稳成退让/跟牌9砸/连拔K/K对侧引小+九砸标记/对象已现清登记/防守方领出接应None(BUG-2)/九砸超吃补登记/A已砸领出无强制干预/stable_make全体口径/探针空仍命中/AKQ顶张齐全不走9砸）；`test_probe_finesse.py` 回归 8/8；真实管线实测（PlayService 直连 DD 引擎）：♠AK8765+♠432 联手 9 张缺 Q 持 AK → `[9砸] 砸A` + 登记 cash_bank → ♠A 入墩南再领出 → `[9砸连拔] 连拔K`（不受稳成线约束，规则优先）；8 张联手场景验证分支优先级（9砸 判据不满足正确落到飞牌介入 `[窗口期启动飞牌]`）；后端 8003 + 前端 5173 构建启动正常
...
**改进**:
- **座位解码重写**：`D2DEALER = {"1":S,"2":W,"3":N,"4":E,"w":S}` + 段序固定 `["S","W","N","E"]`；3 段缺第 4 家（东）由剩余牌推算、4 段残缺补全逻辑保持。dealer 直接读取，删除原 4 假设枚举 + 南庄消解整段逻辑
- **叫牌序列截断**：讲解流会把 what-if 假叫牌（`6H p p p` 后又现 `2C P 2D...`）混入 `mb|` 令牌，导致 2-C9/3-C9/4-C9/5-B9 定约错乱——新规则：先截到 `ha|` 令牌，再在其中有数字叫品后遇 3 连续 pass 即终止
- **庄家**：标准规则（定约花色被庄家方首叫者），不再需要歧义消解

**修改文件**: tools/bm_import.py, bm_deals.json（重生成）, CHANGELOG.md, DEVELOPMENT.md, docs/BM2000读取-教训与交接.md

**测试验证**: 537 副**全部南庄**（吻合 BM2000 南视角做庄训练）、首攻 529 副全为西、0 渲染失败、HCP 全=40；讲解首攻文本（"Xxx leads ... against"）一致率 **443/443**；.Fan 打牌序列完整模拟（跟牌强制+将吃+赢家轮转）join **49 命中、0 多重命中、0 错误首攻方向**（较 v1.83 的 43 个更多，零误报）；2-B29=3NT 南庄（南 8 HCP、明手北 20 HCP，(北)1♥-(南)1NT-(北)3NT 合理）、5-A19=5♣ 南庄西:K♥（西开 4♥ 8 张套、北 18 HCP 加倍、南 7 HCP 牺牲叫，讲解全吻合）；后端 API 实时读文件验证通过，无需重启

## 2026-09-17（BM2000 导入正确性修复：字典序命名 + 首攻重写 v1.83）

**背景**: v1.82 的 538 副数据存在系统性错位——用户实测 2-B20/2-B21 与 BM2000 原局手牌/叫牌不符。根因有二：(1) 旧实现把 Index 偏移当 Data 定位（probe25/26 双重证伪，偏移 2.5M 远超 Data 395KB），"就近找 marker"策略导致名字与牌局错配；(2) 交接文档中的"B20=marker89"锚点本身是旧错误代码的输出（毒锚点），曾误导字典序映射被否决。

**破解（.Fan 会话文件）**：`lines/<lvl>/<课程>.Fan` 是用户学过该课后留下的打牌记录，58 字节 = 6 字节头 + 52 字节牌码（`value = 0x34 + suit_idx*20 + rank_idx`，SHDC 自然序、牌值升序），body 即完整合法打牌序列。用模拟器（leader/trump 枚举+跟牌强制）join 全部 Data marker：L2/L3 各 21 个 .Fan 唯一命中、零错命中、零冲突，全部吻合同一规则——**Data 中 marker 顺序 = Index 纯名字的字典序（字符串排序）截断到 marker 数**。

**改进**:
- **映射规则**：各 level `pure = sorted(纯名字)`、`assign = pure[:有效marker数]`——L1 183 全活（A36+B30+M117）；L2-L4 各 96 活（A+B+C）、D 系 30 死名；L5 66 活（A+B）、C 系 30 死名。id 恢复真实命名（`2-B20` 等三段式 `{层}-{字母+序号}`），死名剔除（120 个）
- **首攻提取重写**（旧版 200 字节窗口误抓讲解高亮、L1 大量缺失）：三级候选 + 首攻者（庄家顺时针下家 `NEXT[declarer]`）手牌归属验证——(1) 最后 `cr|` 后首个 `at|` 前的 `hc/pc`（L2-L5 标准位）；(2) 全文第一个 `pc|XX|`（L1 讲解流，打牌序列从首攻开始）；(3) 第一个 `pc|<花色>|`（L4 只标花色不标牌张时，该花色唯一张可推断）。多解歧义按规则优先级+南庄消解
- **手牌补全**：3 副 md 第 4 家残缺（`1-A31`/`4-A16`/`5-A23`，如东家仅存 `s3` 一张）按"缺牌补给唯一不完整家"推算
- **独立化**：`_bm_convert.py` 依赖函数全部内联进 `tools/bm_import.py`；清理 47 个 `_bm_probe*.py` 等临时探针脚本

**修改文件**: tools/bm_import.py（重写）, bm_deals.json（重生成）, CHANGELOG.md, DEVELOPMENT.md, docs/BM2000读取-教训与交接.md（结论补记）

**测试验证**: 537 副全量渲染 0 失败、HCP 全部=40、529/537 有首攻（8 副数据本身不记牌张：4-A9 等）；B20=4♠南庄首攻西:K♥（与 .Fan 一致）、B21=3NT南庄西:Q♥、1-A1=西:T♠、4-A15=西:T♥（讲解 "West's opening heart lead"+唯一张推断）；.Fan 端到端验证 43 通过 0 错命中（35 失败均为备选线路，任何 marker 上都打不出，与映射无关）；主题关键词抽查 15 个未中案例主题全部吻合。后端实时读文件无需重启，前端默认 id `2-B20` 直接可用

## 2026-09-16（BM2000 牌局导入：四家/两家可选 + 双视图 v1.82）

**背景**: 用户希望把《Bridge Master 2000》练习题（桌面 `bm2000/hands/{1..5}`）转成系统可读入的牌局（四家手牌/首攻/定约/庄家位置/叫牌），并可在导入时选择只显示庄家+明手两家（single-dummy，防家未知）或全部四家。已破解 BM2000 LIN 编码并拿 Level2 B20 逐副验证。

**改进**:
- **协议破解**：BM2000 牌局存于 `Data`（`st||nt||sk||md|` 分界 + `mb|` 叫牌 + `hc|pc|` 首攻 + `pg|` 讲解）；`md|` 旋转码（1/2/3/4/w→南/东/北/西起始，逆时针展开，常只列3家，缺家由52张补全）；发牌人=首家；庄家=契约方最先叫出最终定约花色者；首攻方=庄家上家→庄家=PREV[首攻方]，枚举发牌人起始位取唯一解
- **批量转换** `tools/bm_import.py` → `bm_deals.json`（538 副，含四家手牌+HCP/首攻/定约/庄家/发牌人/叫牌/doubled）；`{层}-{序号}` 命名，B20 加 `2-B20` 别名
- **后端** `GET /api/bm-deal`（`api/main.py`，`deck_id` + `hand_view`），复用 ImageDealResponse 响应形态；`two_hands` 时被掩蔽=庄家与明手之外两家，手牌置空（single-dummy，与 image-deal 缺手牌显示未知一致）
- **前端** 工具条新增「BM」按钮 → 导入对话框（牌局 id + 「仅庄家+明手两家/全部四家」单选）；`useDealing.handleBmDeal` 复用图片识别加载链（含定约/首攻/叫牌），`api.bmDeal`

**已知限制**: BM Index 偏移超出 Data 体积无法用于定位，故按 `{层}-{序号}` 标识；Level1 因分界标记在二进制中误命中检到 183 副（真实题目 36），全量准确性待用户确认后定夺。

**修改文件**: tools/bm_import.py, _bm_convert.py, api/main.py, web/src/services/api.js, web/src/hooks/useDealing.js, web/src/App.jsx, web/src/components/CardTablePanel.jsx, web/src/components/MainTableArea.jsx, CHANGELOG.md, DEVELOPMENT.md

**测试验证**: B20 后端两视图 curl 验证通过（four_hands 全四家 / two_hands 防家置空，含 4♥/庄家南/首攻西:K♣/完整叫牌）；前端 `npm run build` 通过；后端 8003 健康

## 2026-09-16（DD 多数投票：口径矛盾时多票定选 v1.81）

**背景**: 实测 4♠ 牌局（南 ♠KJT9 ♦KQJT9 ♣A，第4墩领出）反复重打时引擎在 ♦K / ♠K 间抖动——逐世界配对诊断揭示：66.6% 世界两张牌完全等价，24% 世界仅 ♦K 好，9% 世界仅 ♠K 好，♦K 优势真实但单次采样噪声（850 样本 ±6-8%）会翻盘。方案乙（用户定调）：**仅在"做成率 vs 赢墩口径矛盾"时触发多票多数**，飞牌介入优先、一律跳过投票。

**改进**:
- **口径矛盾判据（无阈值，用户定调）**：候选按逐世界 scores 全等分组（KQJ连张/Q已出后KJ 自动等价，不算对手）；**做成率榜 top 组 ≠ 赢墩榜 top 组** → 该局面"到底谁好取决于看哪个指标"，才值得多采几票。实测 60 次矛盾率 20/60，精准命中该牌型
- **多数投票**：每票独立 `search`（自重新采样，按面板计分制选牌），取出现最多者；`_dd_play` 内先飞牌介入、飞牌改选则跳过投票
- **实测效果**（4♠ 牌局，850 样本）：单次 62%♦K → 矛盾触发投票后 **93%♦K**（100 次实测）；非矛盾局面照常单次不减速
- **设置项**：后端 `dd_majority_votes`（1~20，默认 1=关闭，运行时粒子设置 API 实时改）；前端 SettingsPanel「多数投票」滑块（localStorage 持久化 + 300ms debounce 推送后端）
- **配置**：`DD_MAJORITY_VOTES`（默认 1）实例化到 `PlayService.dd_majority_votes`

**排查发现（相关但非本次引入）**: 用户"DK 打不出来"实为**历史保存的旧约束「东♠≥3」（[AI] 误约束）随载入记录恢复**，绕过 v1.80 的 [AI] 过滤，东西黑桃分布被锁死 → ♠K 98%。重新生成约束后恢复。**遗留隐患**：载入记录恢复约束路径未接 v1.80 过滤，旧记录载入仍会复现（待后续处理，见 docs）。

**修改文件**: bridge/play_service.py, bridge/mcts/config.py, api/main.py, web/src/App.jsx, web/src/components/SettingsPanel.jsx, web/src/hooks/useModelSettings.js, CHANGELOG.md, DEVELOPMENT.md

**测试验证**: `python -m py_compile` 通过；前端 `npm run build` 通过；后端重启 8003 健康；粒子设置 API 实时改票数生效；4♠ 牌局 100 次实测 93%♦K；用户重打验证通过

## 2026-09-15（约束来源规则定死：AI 一律不转译 v1.80）

**背景**: v1.79 修复约束系统性失效后，用户实测截屏牌局发现新问题：约束转换 LLM 对 [AI] 来源约定叫（扣叫）仍可提取，把东 3H 含义文本中"对开叫人♠有配合（至少3张支持）"错转成 `♠≥3` 硬约束——既误判叫牌方向（东是西的同伴，3H 本应是♥加叫而非♠扣叫），又违反前端约定"后端仅对非 AI 来源行做约束转译"（`web/src/App.jsx:2150`）。根因：后端 `CONSTRAINT_TRANSLATE_PROMPT` 的"[AI] 约定叫可提取"例外是后加的，与前端约定冲突，截屏（新睿模拟补全含义、全 [AI] 行）时最易踩中。

**改进**（用户定调"AI 一律不转译，要有明确记录；提高 AI 叫牌能力过一段时间再做"）:
- `CONSTRAINT_TRANSLATE_PROMPT` 来源过滤规则：**[AI] 行一律不转译**——含义是 AI 模拟补全（截屏牌局用新睿二盖一模拟人类叫牌补全），非叫牌阶段真实产物，不可信，无论自然叫还是约定叫（含扣叫/4NT-5NT/斯泰曼等）都不提取，constraint 输出空字符串
- 原"仅当含义明确属于约定叫时提取"例外**删除**；转译规则2 [AI] 禁止推导 HCP 保留并注明为冗余兜底
- 与前端注释 `后端仅对非AI来源行做约束转译`（App.jsx:2150）对齐，消除前后端口径分歧

**明确记录（勿再改回）**:
- 约束唯一可信来源 = **叫牌阶段真实产物**（人叫牌或 AI 实叫产生的 [JF]/[XR] 含义）；[AI] 行（模拟补全/解释性含义）一律不进入约束
- 提高 AI 叫牌能力（让 [AI] 含义更可信）是**另一项独立工作**，届时重新评估约束策略，不在本次范围内

**修改文件**: bridge/play_service.py, CHANGELOG.md, DEVELOPMENT.md, docs/约束来源分级废弃记录.md

**测试验证**: `python -m py_compile` 通过；后端重启 8003 健康；截屏牌局约束不再出现 ♠≥3 类 [AI] 来源硬约束
...
**改进**:
- **9砸 领出侧补全**：在"引擎榜首已在该花色"路径插入 `_nine_suit_should_garrison` 检查，触发双闸 = 联手 ≥9 张（`_combined_suit_count`）+ 顶张齐；顶张在手 → 改出顶张砸；顶张在对侧 → 改引该花色最小 ≤9 小牌让对侧持 A 方本墩第三家超吃（登记 `flow_extra[suit]["九砸"]`）
- **9砸 连拔（方案A，用户定调）**：缺Q持A+K 分砸 A、K 两墩——砸 A 时登记 `nine_cash_bank[suit]={"obj","rv":13}`（"九砸余顶"），下墩同花色领出、对象未现身、本方可出 K → 改出 K 连拔（第二墩砸）；对象现身/本侧无牌可出 → 清登记正常接管；缺K持A+Q 先砸 A 再飞 Q（既有 `"9砸先飞"` 保留）
- **9砸 接应侧**（`_finesse_commit_check`）：同伙引小、`flow_extra[suit]["九砸"]` 成立、本家持顶张 → 强制用最大顶张超吃（规则优先于威胁判定）
- **探针"全"逐条展开**：`_detect_finesse_struct`/`_probe_partner_finesse_struct` 顶部输出 `"全"`；结构池按"全"逐条 `(对象, 引牌)` 各过 `_probe_finesse_ok`，通过者才入池；`_probe_confirm` 键 `(侧|花色|对象|引牌)` 避免同引牌多对象撞键（J3/K3 曾共显 ✗）
- **`_probe_finesse_ok` 扩展（用户修订）**：G 候选 = 对侧全部牌 ∪ 引牌本身（引牌本身满足 `obj > 引牌 > 防家非对象最大牌` 也构成飞；是"引牌本身"而非引牌侧整手）
- **滑动窗口剔除己方现手持有**（`_honor_missing_of_state`）：先剔己方持有（不占窗口名额、窗口自然下移）再取未打出前 3 张；与 9 砸扩展共同解决 B11 双飞问题
- **引牌选择量化取档统一**（`_VAL_QUANT=0.02`，Δ 平局）：同一 `(花色, 对象, 侧)` 多条达标引牌按"做成率取档量化、Δ 平局决胜"定代表；对象/侧之间仍按 Δ 取最大；`_BUCKET` 改名 `_VAL_QUANT`（"分桶"=位置敏感度、与"取档"=量化容差区分）

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, web/src/components/play/shared.jsx, docs/飞牌讨论与修改记录_20260913.md

**测试验证**: `python -m py_compile` 通过；后端重启 8003 健康；重打验证 9砸 连拔、顶张对侧 9砸 引小/超吃、探针全展开多对象确认
...
**改进**:
- **滑动窗口统一 7 张 AKQJT98（用户定调）**：`_honor_missing_of_state` 窗口 = 该花色未打出前 3 张，持久于 `state.finesse_windows[suit]` 逐花色独立维护、每次检测更新；去掉 flow/非 flow 分支；对象名映射补 9/8（后端+前端）
- **组合飞探测门（用户定调）**：花色内无单条 Δ≥0.4 时，≥2 对象各取最高 Δ 加和 ≥0.4 → 保留较高对象（按 RANK_ORDER 数值，修字符串比较误取 Q 的 bug），Δ 改写为加和值、标记 `组合飞`、其余对象记 `废弃对象` 废弃；`_probe_finesse_ok` 与 `_finesse_commit_check` 威胁计算均剔除废弃对象（后者经 `finesse_flow_extra` 传递）；前端组合 Δ 蓝色加粗
- **探针判定收敛一次**：删除独立 confirm 循环，pool 判定结果同时写 `_probe_confirm`（含 False）与结构池，杜绝"显示✓实际判废"；`_detect_finesse_struct`/`_probe_partner_finesse_struct` 对组合飞/废弃对象回退 "全"[0]
- **修复 RecursionError**：`_register_finesse_flow` 方法体首行恢复直接赋值；`_dd_play` 异常分支补 `[DD_ERROR]`+traceback 诊断（此前静默降级难排查）
- **启动门控阈值**：`FINESSE_NEC_MAKE` 0.85 → 0.50（A1 契约必要放宽）
- **探针展示格式**：单行连排 `本侧 - ♦K,Δ1.04,♦Q,✓ | ...`（段内逗号、段间红色加粗竖线、✓/✗、同花色引牌连排、位置列 本侧/对侧）
- **讨论决策（待实现）**：续飞流程移除方案（每领出重探+重闸、登记存活到本墩出完即取消）；**顶张方概念废弃**（飞牌可行性唯一判据 `_probe_finesse_ok`，勿再引入，已入项目记忆）

**修改文件**: bridge/mcts/dd_search.py, bridge/play_service.py, config.py, web/src/components/play/shared.jsx, docs/飞牌讨论与修改记录_20260913.md

**测试验证**: `python tests/test_probe_finesse.py` 8/8；`python -m py_compile` 通过；前端 `npm run build` 通过；后端重启 8003 健康；重打验证：组合门 `[组合飞探测] ♥ K/Q Δ加和达标、保留较高对象K`、T7/95 出 `♦9 达标` 探针、T7/95 与双飞局面无 `[探针废弃] ♥`、无 RecursionError

## 2026-09-13（探针飞牌结构判定纯条件化 + 过手流程重构 + 打牌输出显示优化 v1.75）

**背景**: ①探针高 Δ 只是"位置敏感"信号，此前以单套 minimax 比"对象放防家两侧赢墩"判定能否飞，总墩/间张墩口径被 5/6/7 冗余中牌补位掩盖，误判 K567/234 对 A；用户定调**纯条件判定、不做赢墩推演**：引牌侧对侧存在 G 满足 `obj > G > 防家除 obj 外最大牌` 即飞结构；②过手逻辑嵌入启动中途，过手后仍写 flow（将吃拦截即留下死流程），用户定调**过手不启动飞牌、预检前置、过手后重新探测走完整流程**；③多结构启动此前取 Δ 最大单条、丢榜首一致花色，误选红桃4（57%）而非引擎榜首草花（75.8%）；④成约率制下 ♣A 9墩 与 ♣5 10墩 同 100% 平局任意排序，选到浪费顶张的 ♣A。详见 `docs/飞牌讨论与修改记录_20260913.md`（新档）。

**改进**:
- **探针飞牌结构判定纯条件化（`_probe_finesse_ok`，用户定义）**：对侧存在 `obj > G > 防家非对象最大牌` 的飞张即保留探针；删除单套 minimax（`_single_suit_decl_tricks` 整体移除）——不再以赢墩推演判飞
- **窗口期启动流程重构（用户定序，BM2000 L2 A-300 验证通过）**：`过手预检`（伙伴侧条目先求安全过手牌，失败出局/成功挂"过手牌"）→ 7.1 引擎一致（榜首∈池→尊重引擎）→ 7.2 排序（代表牌=本侧引牌/伙伴侧过手牌 的候选决策值主 ±0.02、Δ 平局）→ 7.3 必要性（`_finesse_launch_worthwhile` 逐花色，首过=第一名）→ 7.4 本侧直出+写 flow / 伙伴侧出过手牌不写 flow → 7.9 9张分砸分流保留
- **多花色结构池**：全部通过测试的探针花色保留（同花色取 Δ 高者），不再压 Δ 最大单条；"引擎一致优先"由此自然生效（榜首草花在池 → 尊重引擎，不再改飞红桃）
- **过手不启动飞牌**：`_partner_overhand_action` 不再写 `finesse_flow`；过手牌排除所有探针结果花色（`_cash_reentry.excluded_suits`，来源 `_probe_suits`），找不到安全过手牌则该花色出局/尊重引擎——规避过手牌被将吃拦截后死流程麻烦
- **成约率制超额赢墩决胜**：make_rate 决策值 `blended = rate×10000 + avg_tricks`（♣A 9墩 vs ♣5 10墩 案例），同为成约时选赢墩多者
- **探针展示增强**：探针行合入印章流（本侧"探针"+伙伴侧"伙伴探针"）、同侧同对象取 Δ 高、逐条标注 `✓飞结构 / ✗未确认`（`_probe_confirm`）；启动比率诊断日志 `[启动比率]/[过手预检]`
- **打牌输出显示优化**（barchart 与视图）：
  - 同时显示 `墩数[min-max] · IMP · 做成率`，当前计分制加粗；赢墩范围并入墩数
  - 全赢/临界/全输样本子集筛选（后端 `subsets`/`scores_win|crit|lose` 分桶 + 前端按钮，切换重排）
  - IMP 双向条：0 点随数据动态定位、负值向左红色、两侧同背景轨道；bar 固定宽 360px 防挤压
  - DD 移除"DdSummary/均匀采样"独立行（source 合入 DDMC 标题行）；αμ 视图同样原则（核心逻辑/局面评估去重、候选对比与 DDS 诊断折叠、标题带 source/M）
  - 飞牌/续飞印章单行化（标签+内容一行）；打牌计时合并进标题栏按钮区
- **回归测试**：新增 `tests/test_probe_finesse.py`（8 用例：AQ2/43、K567/234、Qx对AJx、AJ对K、AKJT98/32 等，直接运行）

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, web/src/components/play/{shared,CandidateBars,engineViews,utils}.jsx/.js, web/src/components/PlayDetailPanel.jsx, tests/test_probe_finesse.py（新增）, docs/飞牌讨论与修改记录_20260913.md（新增）

**测试验证**: `python tests/test_probe_finesse.py` 8/8 通过；`python -m py_compile` 通过；前端 `npm run build` 通过；后端重启 8003 健康检查通过；BM2000 Level 2 A-300 过手场景实测通过

## 2026-09-12（DD 枚举/采样路径合并 + 枚举 void 硬事实 + 约束联手点力口径 + 飞牌比值退让统一 v1.74）

**背景**: ①用户复现"第 10 步改变计分制"（枚举路径是 endplay 时代复制品，此后 8 个版本两处重复修改漂移——计分制只进采样路径，枚举仍纯 avg 选牌）；②枚举不检测 void 硬事实，含"西持♣"假样本决策错选 ♣A；③约束 LLM 把"联手点力≥37"写成北个人 HCP≥37；④强制接应无比值退让（赢墩制 13vs12 比值失真）；⑤窗口期启动因"跳过榜首花色"误启动 Δ 更小的其他结构；⑥飞 K 时己方 ♦A 已出仍不除流程。详见 `docs/飞牌讨论与修改记录_20260912.md`（新档）。

**改进**:
- **DD 枚举与采样路径合并**（用户定调"要合并不是对齐"）：提取 `_solve_worlds`（批量求解+串行降级+耗时统计）/`_finalize_decision`（选牌/排序/计分制格式化）/`_fmt_score`，两路径唯一差异=世界来源；枚举补 `scoring_val`/`scoring_mode`/`iters_per_sec`；枚举改走 batch 求解，同场景 4.9s→0.1s
- **枚举 world void 硬事实过滤**：复用 `collect_voids`（belief.py）+ 新增 `_hand_violates_void`，垫过牌的花色不再分给该位置；真局 3 样本→1（西♥8♦7/东♣Q），错选 ♣A → 正确改出 ♣8
- **约束"联手点力"转译口径**（`CONSTRAINT_TRANSLATE_PROMPT` 规则2）：联手总点禁止直接转写为个人 HCP；[XR]/[JF] 可按"联手下限−同伴区间上限"推导本家（北 7NT：37−21=16+）；[AI] 一律禁止推导
- **飞牌比值退让统一（两段式 `_finesse_ratio_ok`）**：做成率优先（success_rate / scores≥所需墩）、决策值兜底（`scoring_val`→`avg_tricks`→scores 平均，补宕深浅/超墩感知）；窗口启动 D 闸 0.90、强制接应/8飞9砸/续飞/回手 0.75；2026-09-09"一律强制"设计废止
- **窗口期启动尊重引擎**：榜首已落在任一飞牌结构花色（本侧+伙伴侧合并池）→ 直接尊重引擎，不再因 Δ 排序跳过榜首花色误启动其他结构（♦Q 被改 ♣3 案例）
- **流程清除双原则（`_finesse_flow_dead`）**：①对象已现身（既有）②己方联手现手已无高于对象的牌（上方控制全出，飞无可飞）→ 均清 flow；应用 `_apply_flow_continuation` 清理段与 `_merge_finesse_flow`
- **流程迁移**（用户提出）：比值退让/无牌导致尊重引擎后，引擎实际出牌落在飞牌结构池的另一花色（如退让后出 ♣Q、♣ 也是探针结构）→ 取消原花色 flow（记 `finesse_flow_ends`）、登记实际出牌花色 flow——此前下一墩续飞仍强改原花色（♦9 58.5% vs ♣3 75.2% 案例）；此后再领出该花色为"正在飞"尊重引擎

**修改文件**: bridge/mcts/dd_search.py, bridge/play_service.py, docs/飞牌讨论与修改记录_20260912.md（新增）

**测试验证**: `python -m py_compile` 通过；`tests/test_alpha_mu.py` 全过；枚举路径真局复现（3样本→1、♣8 +10IMP vs ♣A −2IMP）；比值退让六场景复现（做成率/双失败/仅决策值/αμ）；窗口启动与续飞退让两场景复现；后端重启 8003 健康检查通过（`python -m uvicorn api.main:app --host 0.0.0.0 --port 8003`）
...
**改进**:
- **可飞性校验删除**（`_probe_struct_playable` 整体移除）：探针 Δ≥0.4 即采纳为结构。静态"间张≥10 + 上方控制>对象"属强制性定义——牌局在变化，大牌出去小牌就成了大牌；误出防护已由执行层承接（接应威胁比较制"选不出牌就尊重引擎"，不会误改出小牌）
- **接应尊重引擎唯一判据**：`_finesse_commit_check` 返回 None（本家无牌可压威胁/对象已现身/同伙未引飞）才尊重引擎；**比值退让删除**（2026-09-09）、AK 双顶张判据维持删除（09-08）——比值是采样口径（对象位置敏感），不能否决已确定的结构动作（6♠ 例 ♠Q 已现、威胁清空、引牌必胜 → 强制 ♠8 保留 ♠A 控制 K）
- **跟牌接应 flow 补构撤销**：严格只信当墩探针（`_detect_finesse_struct`），判空即无结构不接应；流程延续（finesse_flow）只管下一墩领出，不与接应判据混层（补构属"迫不得已的办法"，用户明确否决）
- **接应结构来源与执行层共用**：`_apply_finesse_commit` 与 `_finesse_commit_check` 共用同一传入 finesse_struct，不再内部重复 `_detect_finesse_struct`——内部二次检测会把调用方已确认的结构静默丢弃（6♠ 例 ♠8 接应被吞的直接根因）
- **探针门控修正（主 search + 残局枚举统一）**：仅"本家正在领出"才探测——跟牌/垫牌时探针会因滑动窗口下移发明新对象（飞 Q 时 Q 刚出 → 滑到"飞 T"），把本应放小的接应误判成必须盖 T；领出方为庄/明手才探测（对方领出不探测）
- **DD 飞牌管理开关**（`DD_FINESSE_ENABLE`）：config + `GET/POST /api/play/dd-finesse` 运行时切换，关闭后 DD 仅按引擎得分选牌（探针一并关闭），αμ 不受影响；前端设置面板 checkbox + localStorage 持久化
- **探针 Δ 阈值运行时滑块**：`FINESSE_PROBE_DELTA` 0.2~0.5（默认 0.4）`GET/POST /api/play/dd-finesse-delta` 即时生效免重启；设置面板对应 RangeSlider

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, config.py, api/main.py, web/src/App.jsx, web/src/components/SettingsPanel.jsx, web/src/hooks/useModelSettings.js, web/src/services/api.js, docs/坐庄决策_双明手全知偏差与单明手精修方案.md（新增）, tests/_tmp_follow_probe_gate.py（新增）

**测试验证**: `_tmp_follow_probe_gate`（3NT 例构造：南♠7 领出、西♠Q、北♠J2 跟牌——第一墩补偿后北跟牌 finesse_probe 为空、无"飞牌接应"字段、尊重引擎不误出 ♠J）PASS；后端重启后 8003 健康检查通过

## 2026-09-08（飞牌启动重构：拖延废弃 + 窗口期主动启动 + 探针滑动窗口与可飞性校验 v1.70）

**背景**: 复盘 6NT/6♠ 等定约发现飞牌启动过晚：探针识别出结构时引擎常先兑现顶张、窗口关闭后飞牌无收益；顶张方自己领出方向错误；且探针一度把"位置敏感但无张可飞"的花色（如 ♠AK+xx 缺 T）误判为飞牌结构，强改出 ♠2 造成伤害。承接 09-06 口径纪律（决策干预无用、行为决策看概率最优），本轮把飞牌干预重构为"探针检测 + 显式验证"两层。

**改进**:
- **拖延策略整体废弃**（用户决策，09-07 删配置、09-08 清残留）：坐庄方明手牌可见、防守方为采样平均，单步贪心不会因拖延换花色"获得信息后改判"——拖延只有代价无收益。删除 `_defer_candidates`/`_is_danger_suit`/`_is_finesse_struct_card` 方法与入口领出拖延分支、配置 `FINESSE_RATIO_RISK`/`FINESSE_LOSE_TRIGGER`/`FINESSE_DEFER_WIN_MIN`
- **窗口期主动启动飞牌**（`_probe_lead_finesse_prefer`，由 9张飞牌优先推广）：探针识别出结构（Δ≥0.4）即窗口期，领出方为庄/明手且引擎榜首是其他花色时主动改出该花色启动飞牌流程。**门槛用探针结构本身（Δ≥阈值）而非 0.95 单步比值**——单步贪心眼里顶张恒高于飞张小牌，走 0.95 会系统性扼杀所有主动启动；9张以上按砸/飞分流，<9张只飞不砸，顶张方无稳赢回手则不强制
- **顶张侧过手启动**（`_apply_lead_transfer`）：当前侧探针未识别结构时，以队友为领出方真跑一次 DD 评估（`dd_search.search` 新增 `perspective`/`actual_turn` 覆盖参数），取队友侧探针同一 Δ≥0.4 门槛（口径一致，模板法只归 αμ）；结构成立且当前方是顶张方 → `_cash_reentry`（原 `_nine_cash_reentry` 更名统一）过手给队友首引飞，写 finesse_flow 标记
- **流程标记对象化**：`state.finesse_flow` 存对象 int，清理只按"对象现身/花色已死"，不再被"当墩探针判空"误清（6♠ 回手后探针一时 Δ<0.4 被误清、北被放行乱出的根因）
- **DD 引擎飞牌识别只用探针法**：probe 键存在（无论是否为空）只用探针结果，判空即无飞牌结构，不回退模板法；αμ 引擎仍用模板法（静态间张）
- **探针 Δ 阈值调至 0.4**：基于真实触发时点（游戏进行中我方赢下上一墩后领出）实测，8张平均 Δ=0.76、9张平均 Δ=0.86；原 0.5 致 8/9 张识别近乎失能
- **可飞性校验**（`_probe_struct_playable`）：探针 Δ 高只说明"位置敏感"，不必然可飞——须有 <对象 且 ≥10 的间张 + >对象 的上方控制才构成可飞结构；主路径与顶张侧过手路径同口径共用，过滤 ♠(T) 类假阳性
- **探针 3 张滑动窗口**（修正固定 AKQJT 五张集实现）：默认监控 AKQ；仅对飞牌花色（finesse_flow 中活动流程的花色）在窗口内大牌打出后下滑补齐三张（K出→AQJ、A出→QJT），支持残局飞 J/T；非飞牌花色固定 AKQ 不往下移，消除"J/T 位置敏感"假阳性
- **6NT ♠(T) 误判修复**：南♠AK32♥K32♦432♣KQJ / 北♠54♥AQJ♦AQT987♣A2 此前误改出 ♠2（探针报 ♠T Δ=0.4 压线）；修复后只识别真实 ♦(K) 结构（9张缺K持AQ → 8飞9砸先砸A再飞Q），12 墩做成
- **接应选牌"威胁比较制"**（`_finesse_commit_check` 重写，废除 10 分界）：先处理敌方本墩已出对象 → 最小顶张盖；对象历史已现身 → 不贴小牌尊重引擎；否则比较同伙引牌与"敌方剩余牌（全部牌面−我方现手−已出）中除对象外最大者"——引牌更大 → 出最小牌保留结构；引牌不足 → 出大过威胁的最小牌（第三家打大牌），无牌可压 → 尊重引擎。双间张先飞低张、不烧回手间张
- **跟牌接应侧 flow 补构**：`_apply_finesse_tactics` 跟牌分支探针判空但有活动 finesse_flow 时用 flow 对象补齐结构再判接应（与领出侧同口径，避免探针采样波动中断接应）**【2026-09-09 已撤销：严格只信当墩探针，见 v1.71】**
- **接应尊重引擎退让裁剪为单判据**（删除 AK 双顶张"砸路线"判据）：原①接应方持 >对象 顶张 ≥2 视为砸路线尊重引擎——适用面窄且联手总张数少时常误判（双顶张在手也可能真需飞，尚缺中间张），删除后统一按威胁比较制强制接应；仅保留②接应飞张与榜首决策分比值 <0.90 时尊重引擎**【2026-09-09 比值退让一并删除，尊重引擎仅剩"选不出牌"，见 v1.71】**

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, bridge/play_types.py, config.py, tests/_tmp_engine_run.py, tests/_tmp_fly_check.py（新增）, tests/_tmp_probe_window.py（新增）, tests/_tmp_commit_check.py（新增）

**测试验证**: 滑动窗口单测（无流程 AKQ 窗口/飞牌花色 K 出后滑到 J/非飞牌花色 Q 出不补位）PASS；`_tmp_fly_check`（♠(T) 排除、♦(J) 保留）PASS；竞应威胁比较制 5 用例（盖K/压威胁/引牌胜最小跟/无牌可压/对象已现）PASS；seed42 6NT 回归 12 墩做成、无假 ♥T 过手/♠T 启动；后端重启后 8003 健康检查通过

## 2026-09-06（6♠ 复盘研究：否决清将干预——局部 vs 全局口径）

**背景**: 复盘 6♠（南庄 ♠AK432♥A32♦A32♣32 / 北 ♠Q65♥KQJ♦54♣AK654）失败局，初判"DDMC 未连续清将导致超将吃"为引擎缺陷，一度设计清将启发层方案（`.trae/documents/trump-control-heuristic.md`）。经全局研究后结论反转：**口径错误导致的伪命题，引擎决策正确，不应干预**。

**研究结论**:
- 全局双明手做成率 ≈ **60.75%**（4000副采样）：将牌3-2+草花OK 99.9% 可成；将牌 4-1/5-0（占 32%）无论草花基本不可成——失败主因是将牌分布，非草花
- 整副模拟（庄家=DDMC，防守=双明手最佳，同种子对照，40副）：可成子集无干预 **97.5%** vs 清将干预 **87.5%**；全分布 50.0% vs 47.5% → **清将干预净损害，方案否决**
- 失败局 2.5% 是引擎单盲采样承担 4-1/5-0 分布风险的概率性代价，属"长期期望正确"的固有支出
- **口径纪律**：局部单步"做成率低" ≠ 全局策略无效；`avg_tricks` 非决策口径（系统用 `scoring_val` imp/make_rate）；"能成的存在性" ≠ "当前步该清将"

**修改文件**: 无源码改动；新增 scripts/_verify_dd_make.py, _verify_dd_global.py, _verify_make_rate.py, _verify_trump32_subset.py, _verify_global_intervention.py（研究验证脚本，保留）；docs/6S清将干预研究_局部vs全局口径.md（新增报告）

## 2026-09-03（飞牌策略体系 + 体系标注 + DD样本开关 v1.69）

### 飞牌策略体系：识别/接应/拖延/8飞9砸 + DD世界过滤

**背景**: 引擎（DD/αμ）在处理单套飞牌结构时按期望赢墩平均决策，缺少桥牌飞牌常识——识别可飞结构、接应飞张优先、可拖延时搜集信息、联手张数决定砸/飞。承接 2026-08-26 研究（`docs/飞牌vs拔AK_4H双飞时序研究.md`），建立与引擎平级的飞牌干预管线。

**改进**:
- **飞牌结构识别（方法B·静态间张）**：合并庄家+明手每花色，从高到低找"可飞对象"缺失大牌 M（K/Q/J），我方有上方控制张+下方≥T飞张+合计≥4张 → 该花色可飞（`_detect_finesse_struct`）
- **统一干预管线 `_apply_finesse_tactics`**（独立于引擎，DD/αμ 共用，仅庄家/明手出牌触发）：领出判断是否引发飞牌并记录；可拖延（存在比值达标的无关牌换花色）则拖延避免引发；跟牌强制接应 + 8飞9砸；垫牌不处理
- **8飞9砸原则**：联手张数≤8 → 榜首若为砸张（A/K）且候选飞张比值≥0.95 → 改出飞张；联手≥9 → 榜首若为飞张且候选顶张比值≥0.95 → 改出顶张；差距大（<0.95）→ 不干预（尊重引擎）
- **9砸后续**：砸A后领出方若为顶张方（持 A/Q）一律不得自己出该花色，强制改出无关花色小牌回队友手再飞K（`_apply_lead_finesse_check` 分顶张/非顶张方）；9张缺K持AQ先砸后飞 + 砸A后保护间张
- **飞牌流程跨墩延续**：从回手牌/直飞小牌开始标记 `state.finesse_flow`，流程进行中强制续飞（非顶张方领出强制出飞张小牌、顶张方领出强制回手、对象大牌现身清除标记）；无进手/无多余飞张小牌时交还引擎
- **拖延时机判定**：全赢占比≥80% 视为稳成、早飞禁拖；不足 80% 允许拖延搜集信息；护张保护排除危险花色（联手≤4张且无顶张控制）
- **飞牌接应融合度比值保护**：接应飞张/榜首 < 0.95 判定非融合，尊重引擎不强制
- **8飞9砸联手张数按墩结算**：仅首墩按"在手+本墩我方已出"计 9 张，非首墩按剩余在手张数计算；同伙引小后不再误判 8 飞
- **比值统一**：`FINESSE_RATIO=0.95` 全局统一（撤销 DD/αμ 分常量 FINESSE_DD_RATIO / FINESSE_ALPHA_RATIO），延迟/8飞9砸/9砸后续/接应同源单点改值
- **DD 世界过滤**: dd_search.py 支持按世界类别（全赢/临界/全输）过滤聚合（配合 08-22 的 DD_KEEP_* 开关）

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, config.py, docs/飞牌优化讨论与修改记录_20260830.md, docs/飞牌vs拔AK_4H双飞时序研究.md, docs/单套结构飞牌总谱.md

**测试验证**: 单套结构飞牌总谱全例覆盖；30次重复采样决策点稳定性验证；8飞9砸按墩结算回归（同伙引小不再误判 8 飞）；9砸后续顶张方/非顶张方分支实测

### 叫牌来源标注 JF/XR/AI + X/XX 约定解说 + 默认新睿体系

**背景**: 叫牌含义面板不区分来源，无法区分 JF/新睿/AI 生成的解说；新睿体系试用期要默认启用；X/XX 缺少约定解说。

**改进**:
- `bridge/bidding_service.py` 对每叫品标注来源（JF 检索命中 / XR / AI 生成），前端含义面板按来源着色/标记
- X 与 XX（加倍/再加倍）补约定解说文案（`llm/prompts.py`、前端展示）
- 默认打牌体系改为新睿（XR），`api/main.py` 相应透传

**修改文件**: api/main.py, bridge/bidding_service.py, llm/prompts.py, web/src/App.jsx, web/src/components/PlayDetailPanel.jsx, web/src/context/GameContext.jsx

### DD 样本类别开关前后端接通

**改进**: DD 全赢/临界/全输三类世界保留开关（config `DD_KEEP_*`）接入设置面板，`/api/play/config` 读写 + SettingsPanel 勾选 + useModelSettings 持久化（api/main.py, web/src/components/SettingsPanel.jsx, web/src/hooks/useModelSettings.js, web/src/services/api.js, web/src/App.jsx）

### 历史记录保留修复

**改进**: 后端不可用时 `useBridgeRecords.js` 保留现有历史记录，不再清空（3 行改动）

### 新睿校对配套记录

**改进**: docs/ 新增新睿体系全书校对总结报告、人工校对清单与警告文件（`scripts/xr_data/validate_warnings.txt`）
...
**背景**: 用户实测发现两条约束错误：① `3H→4H` 平加到局被误判为跳加叫，错误套用 16-18HCP 强牌约束；② 应叫人平加叫按 12-15 点、将牌≥4 判定，与"弱牌冲局"逻辑矛盾，且点力只设下限 6+、未封顶。桥牌常识：同伴跳叫原花（≥6 张套）后，应叫人平加到局属弱牌直封——配合张数达标即封局，不再表示额外力量。

**改进**:
- **跳加叫判定改为相对阶数**：对比同伴最后花色叫品的阶数，加一阶=平加、≥两阶=跳加（`3H→4H` 不再误判为 16-18 强牌跳加叫）
- **新增高花弱牌直封特例**：同伴跳叫原花（≥6 张套）后平加到局，点力收窄到 **6-9**，高花将牌只需 **≥2**（6+2=8 张配合即可）
- **扩展低花弱牌直封**：5 阶低花局将牌只需 **≥3**（6+3=9 张配合），点力同样 6-9——修复此前低花冲局误套 16-18 跳加叫约束（如 1♦-1♠-3♦-5♦ 北家被误判 16-18 点、♦≥4）
...
**改进**:
- **约束来源双通道**（按"家"粒度自动切换）：是否有结构化约束段决定走通道A还是通道B，与牌局来源无关，`bid_meanings` 仍旧只作为单一入参
  - **通道A**：叫牌阶段 LLM 在 `叫品含义` 后输出 `叫品约束` 字段（该位置累计至今的自身承诺，按单调收紧不变式生成），`llm/prompts.py` 三处模板新增该字段并给定语法；`main.py`/前端 `formatBidMeaningLine` 拼入 `bid_meanings`（`[约束:...]`）
  - **通道B**：抽出 `_build_constraint_from_meaning_free` 保留原正则解析，行为与旧版完全一致（向下兼容）
  - **结构化家为权威来源**：`play_service._get_bid_constraints` 对含结构化段的家不再与规则库硬约束取交集，避免规则库误判（如第三次叫牌误收紧）污染结构化结果
- **体系适配（消除硬编码）**：`bid_constraint_library.py` 新增 `SYSTEM_XR` 配置；`play_service` 打牌初始化接收实际 `bid_system`（jf/xr/natural）传给规则库，`api/main.py` `PlayInitRequest` 与前端 `playInit`/三处调用点透传该参数
- **`叫品约束` 格式规范**：单行紧凑串，按 `|` 分段——HCP段（`HCP12-21`/`HCP16+`/`HCP≤7`）、花色段（字母 `S/H/D/C`+张数，`+`至少/`-`至多/裸数字精确）、牌型段（`均型`/`非均`/`单缺`）

**修改文件**: api/main.py, bridge/play_service.py, bridge/mcts/bid_constraint_library.py, llm/prompts.py, llm/xr_prompts.py（复用共享模板继承新字段）, main.py, web/src/App.jsx, web/src/services/api.js, docs/叫牌约束提取优化方案.md（新增）, DEVELOPMENT.md, CHANGELOG.md

**测试验证**: 逆叫样例（南1C-北1S-南2D）结构化解得 `HCP16-21·C5+·D4+·非均`（2026-08-21）；北保持 `HCP6+·S4+`——规则库曾在第三次叫牌误判北为 12-15，现在正确未被污染；通道B自由文本回退行为不变（旧缺陷如"8-10点"无法解析为 HCP 恰由通道A弥补）；Python 语法、前端 eslint 0 错误均通过。**效果待实战验证**。
...
### 背景
深入讨论采样器的优化方向，澄清"约束守恒/伪冲突"认知，明确回退（MH→2）的真实来源是可行解稀疏而非约束冲突，并确立"用采样算法近似完整世界、不依赖枚举"的设计原则。

### 结论
- **守恒成立**：两家未知位置牌池守恒，完整手牌满足约束 → 折算后的剩余约束必然可同时满足
- **回退根因**：可行解稀疏（约束越紧，合法世界占比越低），MH 导向性搜索难以命中稀疏解，非约束冲突
- **候选方向**：① 中局约束软约束降级 + 稀疏性量化；② 序贯重要性采样 / MH 提议分布改进；③ 粒子滤波（长期，需解决重采样，避免重复废弃 BeliefTracker 的坑）；④ 低垂果实（约束过度扣减修正、MH 增量 score、β 自适应）
- **取舍**：优先方向 1，投入产出比最高

### 落地：稀疏性量化 + 主从软约束采样
- **量化**（bench_sparsity.py）：逆叫16+ 单约束合法世界占比仅 0.225%（1/444）；双约束（南/西各一主动叫牌）同满足占比低至 0.004%~0.012%。L2 放宽后全部 100%，确认 `relax_constraint` 区分力基本丧失
- **根因**：回退（MH→2）源于可行解稀疏——约束越紧，均匀空间里合法世界占比越低。两家未知位置（通常是防守双方）各有硬约束时，对称 MH 需同时收敛两家，花色/均型互相拉扯更难命中。**不**把"两家同时高 HCP 导致牌池无解"当根因（那种两家都强开叫牌型的叫牌实战中基本不出现）
- **实现**（sampler.py）：新增 `_sample_master_soft`，在 MH→2 或 INFEASIBLE 时选中约束更紧、认知更准的一方（`_master_priority` 排序）单独做 MH 修复，其余位置吃剩余牌、约束只罚分不否决。典型场景：防守双方各承担不同角色且约束不对称（如西 1NT 开叫 15-17 + 东转移应叫 5+♠）
- **验证**：master 约束满足率从 ~0.20%（≈随机）提升到 100%；仅失败路径触发，不影响正常单约束路径

**修改文件**: sampler.py（新增 `_sample_master_soft`）、DEVELOPMENT.md（新增量化结果与落地小节）、bench_sparsity.py（量化工具）

## 2026-08-11（αμ 引擎残局枚举修复验证 + 全流程计时）

### 背景
四家全 AI 叫牌+打牌全流程验证时，αμ 引擎在极深残局（每手剩余 1-2 张）采样因叫牌约束不可满足陷入 MH→Level2→Level0 回退链死循环，牌局卡住不推进。此前会话已对 `play_service.py` 的 `_alpha_mu_play` 添加残局枚举分支、对 `dd_search.py` 的 `_enumerate_endgame` 强制 `constraints=None` 跳过残局约束验证。本次为修复后的完整验证。

### 改进
- **验证脚本 `verify_flow_timing.py` 修正逐墩监听文本**：打牌阶段渲染的是 `PlayDetailPanel`（进度文本"出牌记录 (X/13)"），而脚本原监听 `PlayPanel` 的"已完成墩 (X/13)"，导致逐墩计时永远不命中。改用 `出牌记录 (X/13)` 后逐墩计时正常
- **αμ 引擎残局枚举修复生效验证**：第 8-9 墩（每手≤4张残局）顺利通过 `_enumerate_endgame` 精确枚举，未再陷入采样死循环，13 墩全部正常打完

### 全流程计时（定约 1NT 级，DD-αμ-LLM 引擎）
- 发牌 3.0s；叫牌约 40s；打牌准备 9.7s
- 逐墩：49.9 / 30.4 / 26.2 / 15.7 / 24.2 / 13.3 / 15.6 / 13.0 / 18.9s（递减，符合 DD 自适应采样）
- 打牌总耗时约 294s，总耗时（发牌→打牌结束）337.1s

**修改文件**: bridge/play_service.py, bridge/mcts/dd_search.py, verify_flow_timing.py
...
**背景**: 修复人类/AI出牌响应延迟、AI思考圆圈延迟，并修正DD引擎阶段LLM审查提示词仍写"αμ"的问题

**改进**:
- 人类出牌乐观更新：点击后立即将牌显示到桌面并移出手牌，后端返回后用权威状态覆盖，失败时回退
- AI出牌两阶段：轮到AI立即进入思考状态（中心圆圈马上旋转），250ms后再执行出牌
- ai_play 接口直接返回 state，前端不再额外调 getPlayState，减少一次往返延迟
- 牌桌防闪隐：当前墩为空时直接用 playState.tricks 最后一墩，避免刚出的牌短暂消失
- 已出牌模式优先用顶层完整手牌，避免复盘回放时与已出牌重复计数
- DD/αμ 阶段 LLM 审查提示词 engine_name 通用化，不再硬编码"αμ"
- 移除 DD/αμ 路径两个 _validate_and_fallback 调用

**修改文件**: api/main.py, App.jsx, CardTable.jsx, play_service.py

**测试验证**: 前端 HMR 无编译错误；Python py_compile 通过；后端健康检查 200；DD hint 复盘正常

## 2026-08-07

### 出牌面板隐藏已出/他手的牌 + "显示全部"控制

**背景**: 出牌面板中灰色失效的已出/他手牌会干扰点击，降低输入效率

**改进**:
- 出牌面板默认隐藏已出/他手的牌，仅显示当前可选牌
- 标题栏新增"显示全部"checkbox，勾选后恢复显示全部13张牌（已出/他手的牌以灰色失效状态显示）
- "显示全部"checkbox 为面板级本地 state，切换位置/重开面板时保持

**修改文件**: CardTable.jsx

**测试验证**: 面板默认只显示可选牌，勾选"显示全部"后恢复全部牌

### 打牌完成新增"复盘"按钮

**背景**: 打完牌直接进入本地复盘与"从历史记录载入"两条路径实现不一致，体验不便

**改进**:
- 打牌完成时停留在完成状态，不再自动进入复盘
- 新增"复盘"按钮，功能相当于从历史记录载入最新一条打牌完成的记录，与"从历史记录载入"走同一路径
- 修复复盘按钮回调传递断点：MainTableArea 向 RightPanelSwitcher 漏传 onReviewCompletedPlay
- 载入打牌记录后复盘游标停在全部已出位置，最后一张牌显示在牌桌上

**修改文件**: App.jsx, MainTableArea.jsx, PlayDetailPanel.jsx

**测试验证**: 打牌完成后"复盘"按钮正常显示，点击后载入最新打牌记录并在牌桌显示完整结果

### 载入打牌记录一步直达打牌界面

**背景**: 载入含打牌数据的记录时，会先进入叫牌界面再自动进入打牌界面，体验有跳跃感

**改进**:
- loadRecordToTable 中 setShowPlayPanel 改为按 hasPlayState 判断（含打牌数据 → true）
- 载入记录后自动进入打牌界面的 useEffect 不再依赖 !showPlayPanel
- 仅叫牌完成、未开始打牌的记录仍进入叫牌界面

**修改文件**: App.jsx

**测试验证**: 含打牌数据的记录载入直接进入打牌界面；仅叫牌的记录进入叫牌界面
