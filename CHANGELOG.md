...
**背景**: 用户指出基本不存在三飞，窗口第三对象（通常是 Q，被缺失 A 挤占名额时更是纯浪费）白耗探针分桶计算。窗口 3→2 从 [A,K] 起步，省约 1/3 探针分桶。
...
- **依据**：A 是单花色最大值，`_probe_finesse_ok` 需 `obj > G > max_enemy` 而 G > A 不存在 → A 永远判 False，占窗口名额纯浪费；双飞合并只要求 ≥2 对象（组合飞=双飞语义，v1.90 起定调"桥牌不存在同花色三飞"），2 张窗口覆盖 K/Q、K/9、Q/J 全部双飞组合
- **边界**：A 也缺失（我方无 A 且未打出）时窗口=[A,K]、Q 被挤出——但此时"飞 Q"本无意义（防家 A 会吃），桥牌逻辑无损失
