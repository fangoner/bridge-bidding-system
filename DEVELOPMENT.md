...
### 3. Deep Finesse模块 (`bridge/deep_finesse.py`)
- 定约可行性分析
- 支持当前牌局分析
...
### 6. 历史记录模块 (`utils/history.py`)
- 保存叫牌记录
- 查看、删除、加载历史牌局
- 编辑备注

### 7. 截屏模块 (`utils/screenshot.py`)
- Edge浏览器窗口截屏
- 全屏截屏
- 豆包API识别牌局信息

### 8. 输出格式模块 (`bridge/output_format.py`) - v1.4新增
- 程序化生成三种输出格式，无需AI调用
