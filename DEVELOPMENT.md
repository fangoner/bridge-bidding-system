...
- **背景**：基本不存在三飞；A 是单花色最大值、`_probe_finesse_ok` 需 `obj > G > max_enemy` 而 G > A 不存在 → A 永远判 False，占窗口名额纯浪费
- **改动**：`_honor_missing_of_state` 窗口 `cands[:3]` → `cands[:2]`（AK 起步、出一个向下补一个、下限 8 不变）；双飞合并只要求 ≥2 对象（组合飞=双飞语义），2 张窗口覆盖 K/Q、K/9、Q/J 全部双飞组合，省约 1/3 探针分桶计算
- **边界**：A 也缺失时窗口=[A,K]、Q 被挤出——但"飞 Q"本无意义（防家 A 会吃），无损失
