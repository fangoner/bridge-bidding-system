import os
from pathlib import Path
from dotenv import load_dotenv

BASE_DIR = Path(__file__).parent

# override=True：让 .env 中的配置始终优先于系统/父进程环境变量，
# 避免旧环境变量（如 DEEPSEEK_API_KEY）压过 .env 中的新值
load_dotenv(BASE_DIR / ".env", override=True)

DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY", "")
DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com")

DOUBAO_API_KEY = os.getenv("DOUBAO_API_KEY", "")
DOUBAO_BASE_URL = os.getenv("DOUBAO_BASE_URL", "https://ark.cn-beijing.volces.com/api/v3")
DOUBAO_VISION_ENDPOINT = os.getenv("DOUBAO_VISION_ENDPOINT", "")

DEEPSEEK_VISION_MODEL = os.getenv("DEEPSEEK_VISION_MODEL", "deepseek-flash")
VISION_PROVIDER = os.getenv("VISION_PROVIDER", "deepseek")
DOUBAO_SEED_2_1_PRO_CHAT_ENDPOINT = os.getenv("DOUBAO_SEED_2_1_PRO_CHAT_ENDPOINT", "")
DOUBAO_SEED_2_1_PRO_REASONING_ENDPOINT = os.getenv("DOUBAO_SEED_2_1_PRO_REASONING_ENDPOINT", "")
DOUBAO_SEED_2_1_TURBO_CHAT_ENDPOINT = os.getenv("DOUBAO_SEED_2_1_TURBO_CHAT_ENDPOINT", "")
DOUBAO_SEED_2_1_TURBO_REASONING_ENDPOINT = os.getenv("DOUBAO_SEED_2_1_TURBO_REASONING_ENDPOINT", "")

JF_CONVENTION_FILE = BASE_DIR / "JF实战_标准自然 - Rev 3.2.docx"

DEFAULT_DEAL_SYSTEM = "自然阻击"

DEEP_FINESSE_PATH = BASE_DIR / "Deep Finesse 2014 v2" / "Deep Finesse.exe"
DEEP_FINESSE_CLI_PATH = BASE_DIR / "Deep Finesse 2014 v2" / "df char mode.exe"

SHOW_FULL_LLM_OUTPUT = True

OUTPUT_MODE_GRAPHIC = "graphic"
OUTPUT_MODE_COMPACT = "compact"
OUTPUT_MODE_DEEP_FINESSE = "deep_finesse"
OUTPUT_MODE_ALL = "all"
DEFAULT_OUTPUT_MODE = OUTPUT_MODE_ALL

FALLBACK_MODEL_CHAT = "deepseek-flash"
FALLBACK_MODEL_REASONER = "deepseek-flash"
DEFAULT_FALLBACK_MODEL = FALLBACK_MODEL_CHAT

MAIN_PROMPT_MODEL_CHAT = "deepseek-flash"
MAIN_PROMPT_MODEL_REASONER = "deepseek-flash"
DEFAULT_MAIN_PROMPT_MODEL = MAIN_PROMPT_MODEL_CHAT

MAIN_PROMPT_TEMPERATURE = 0.2
FALLBACK_PROMPT_TEMPERATURE = 0.5

AI_PROVIDER_DEEPSEEK = "deepseek"
AI_PROVIDER_DOUBAO = "doubao"
DEFAULT_AI_PROVIDER = AI_PROVIDER_DEEPSEEK

# ── 模型名称常量 ──
# DeepSeek（2026-09-10 V4.1-Flash 发布后统一正名 deepseek-flash；
# V4 Pro 已于 09-14 下线，DEEPSEEK_MODEL_PRO 暂时同指 flash，
# 未来 V4.1 Pro 上线时改回即可）
DEEPSEEK_MODEL_FLASH = "deepseek-flash"
DEEPSEEK_MODEL_PRO = "deepseek-flash"
# Doubao Seed
DOUBAO_MODEL_2_1_PRO = "doubao-seed-2.1-pro"
DOUBAO_MODEL_2_1_TURBO = "doubao-seed-2.1-turbo"

# ── 统一模型列表（所有可用模型，不含 ::reasoning 后缀的视为 chat 版）──
_DS_MODELS = list(dict.fromkeys([DEEPSEEK_MODEL_FLASH, DEEPSEEK_MODEL_PRO]))
_DB_MODELS = [DOUBAO_MODEL_2_1_PRO, DOUBAO_MODEL_2_1_TURBO]
ALL_BASE_MODELS = _DS_MODELS + _DB_MODELS
# 模型对应客户端类型
DOUBAO_MODEL_NAMES = _DB_MODELS  # 这些模型走 DoubaoSeedClient
DEEPSEEK_MODEL_NAMES = _DS_MODELS  # 这些模型走 DeepSeekClient

def is_doubao_model(model_name: str) -> bool:
    """去掉 ::reasoning 后缀后判断是否为豆包模型"""
    base = model_name.replace("::reasoning", "")
    return base in DOUBAO_MODEL_NAMES

def is_reasoning_model(model_name: str) -> bool:
    """模型名是否带思考模式后缀"""
    return "::reasoning" in model_name

def get_base_model(model_name: str) -> str:
    """去除 ::reasoning 后缀得到基础模型名"""
    return model_name.replace("::reasoning", "")

def expand_model_list(base_models: list) -> list:
    """将基础模型名展开为包含 chat/reasoning 两个版本"""
    result = []
    for m in base_models:
        result.append(m)
        result.append(f"{m}::reasoning")
    return result

ALL_MODELS = expand_model_list(ALL_BASE_MODELS)

# DD play engine settings
# 2026-09-07：MCTS 引擎已移除（能力弱，代码不再保留）
DEFAULT_PLAY_ENGINE = "dd"  # 主力引擎；可选 "llm" | "dd" | "perfect" | "alphamu"
DD_NUM_SAMPLES = 1000  # DD 引擎默认采样数（2026-09-23 200→1000：探针 Δ 门槛稳定化实测）
DD_MIN_SAMPLES = 15   # floor for adaptive sample scaling
DD_TIME_LIMIT = 30.0  # seconds per DD play decision (30秒预算，允许首攻冷启动)
# DD 决策计分制（全量样本口径）："make_rate"（做成率，**默认**）|
#   "imp"（期望IMP，考虑宕分/超墩/局况）| "avg_tricks"（平均赢墩，纯MP思路）
# 2026-09-20 由 "imp" 改为 "make_rate"：使引擎选牌口径与飞牌规则层一致——
#   稳成线 `_stable_make` 与比值退让 `_finesse_ratio_ok` 均以「做成率 =
#   scores ≥ tricks_needed 的世界占比」度量，与计分制无关；imp 模式下引擎
#   可能为追超墩而选做成率更低的路线，与规则层口径打架。
#   make_rate 下引擎决策值 = 做成率×10000 + 平均赢墩（赢墩仅作近似平局的决胜）。
DD_SCORING_MODE = "make_rate"

# DD 多数投票（2026-09-16，用户定判据）：防止"做成率 vs 赢墩口径矛盾"时
#   采样噪声翻牌。触发条件（全部满足）：①飞牌介入未改选（飞牌优先）；
#   ②非等价候选做成率榜 top 组 ≠ 赢墩榜 top 组（等价=逐世界 scores 全等，
#   KQJ连张/中间牌已出的KJ 均视为等价，不触发）；③票数 > 1。
# 每票＝一次独立 search（自重新采样，按面板计分制选牌），取出现最多的牌。
DD_MAJORITY_VOTES = 1    # 多数投票票数；1 = 关闭（默认保持现状）

# DD 样本类别保留开关（默认全保留=不对任何类别过滤，等同原行为）：
#   每世界按"所有候选出牌相对所需墩的情形"分三类：
#     全赢 sure_win  = 所有候选都 ≥ 所需墩
#     全输 sure_lose = 所有候选都 < 所需墩
#     临界 critical  = 有赢有输
#   取消勾选某类 = 将该类世界排除出期望聚合（人工/未来程序根据局面选择组合）
DD_KEEP_SURE_WIN = True
DD_KEEP_CRITICAL = True
DD_KEEP_SURE_LOSE = True

# 飞牌干预统一比值（所有引擎共用同一常量，延迟/飞牌介入同源，
# 修改时只需改一个常量即可全局生效）：
# 以比值（相对成功率）统一跨计分制：改选牌/榜首 ≥ 该值 即视为"差距不大"才干预。
FINESSE_DEFER_ENABLE = True          # 飞牌干预总开关（窗口期启动/接应/流程；2026-09-08 拖延已废弃）
DD_INTERVENE_ENABLE = True          # DD 引擎介入总开关（三段统一：稳成→无损清将→
                                     # 飞牌探针→多数投票；2026-09-22 由原 DD_FINESE_ENABLE 改名，
                                     # 清将/飞牌 共管；同日 9砸 分支移除——DD 成约率口径已覆盖
                                     # 抓Q/顶张兑现）。关闭后 DD 仅按引擎得分选牌，
                                     # αμ 不受影响；运行时切换
DD_USE_CONSTRAINTS = True            # 打牌决策是否使用叫牌约束（采样世界分布/LLM提示注入）；
                                     # 关闭后所有引擎按无约束均匀采样（运行时切换）
# 首攻长四约束（v2.10）：NT 定约防家首攻小牌时，按"长四"协议调整首攻家约束：
# 该花色长度 ≥4 且至少 3 张同花色大于首攻牌；与叫牌约束单调合并（只缩小不扩大）。
# 首攻牌已打出，剩余约束由中局扣减（_reduce_constraint_for_played）自然承接。
DD_LEAD_LONG_FOUR_ENABLE = True      # 首攻长四约束总开关（仅 NT + 防家首攻小牌触发）
DD_LEAD_SMALL_RANKS = {"2", "3", "4", "5", "6", "7", "8", "9", "T"}  # 视为"小牌"的首攻牌面（长四适用）
# 顶张连张首攻约束（v2.14）：NT 定约防家首攻顶张大牌时，按"连张"协议强制该花色
# 必持下一级大牌——攻 K ⇒ 必持 Q（KQ 连张），攻 Q ⇒ 必持 J（QJ 连张）。中局扣减与
# 长四同路径（specific_cards 由 _reduce_constraint_for_played 承接）。攻 A 的"带 K"推论
# 在 NT 下较弱（部分打法攻 A 表示无 K），本次不做。
DD_LEAD_HONOR_CHAIN_ENABLE = True    # 顶张连张首攻约束总开关（仅 NT + 防家首攻顶张触发）
DD_LEAD_HONOR_CHAIN_NEXT = {"K": "Q", "Q": "J"}  # 首攻牌 → 承诺该花色必持的下一级大牌
# 有将首攻约束（v2.15）：4♠/4♥ 等有将定约防家首攻 → 按表12-3/表12-4 生成约束，
# 顶张（A/K/Q/J/10）与 NT 不同（K 行含 AK、Q 行为 QJ10+），小牌走 3/5 首攻
# （奇数攻最小、偶数攻第3大、双张攻大、3张带大牌攻最小/三张小牌攻中间）。
DD_LEAD_TRUMP_ENABLE = True          # 有将首攻约束总开关（仅非 NT + 防家首攻触发）
FINESSE_RATIO = 0.75                 # 飞牌干预统一比值（DD 三种计分制 / αμ 引擎共用），
                                     # 以比值（相对成功率）统一跨计分制：改选牌/榜首 ≥ 该值才干预
                                     # （0.95→0.75，不随 B26 收紧——比值退让的代价悬殊场景
                                     #  改由"引擎榜首同花色即采信"新机制处理（_finesse_commit_ratio_ok，
                                     #   2026-09-18）：引擎 top1 已在飞牌花色 → 直接选 top1，避免
                                     #   强制改选破坏引擎规划；比值本身维持 0.75 宽口径）
FINESSE_PROBE_DELTA = 0.10           # 飞牌后果敏感性探针阈值（v1.92 口径）：缺失大牌在
                                     # 东/西两桶的做成率差 ≥ 此值才判该花色为飞牌结构
                                     # （原赢墩均值差口径信号上限=做成率差×生死线墩差，
                                     #  临界定约（满贯生死线常 1 墩）必失明；改成率口径后
                                     #  v1.93 由 15pt 降至 10pt：425 世界桶 n≈200 的采样
                                     #  噪声约 ±5pt，真信号（如小♦ 引牌 14.8pt）曾骑在
                                     #  15pt 阈值上两测 16.1/13.5 摇摆漏检；Δ 取幅值做
                                     # 进池门票，方向另存探针"方向"键）
FINESSE_COMMIT_ALIVE_PCT = 0.50      # 接应退让判据（2026-09-25 用户定调，删 DIE 强制分支）：
                                     # 押注方向桶内引擎最优替代牌做成率 ≥ 此值 且 ≥ 非押注桶
                                     # → 定约不依赖这墩飞牌，退让引擎（B26 型：西桶 ♦A=0.542）；
                                     # 不满足（含桶数据缺失）→ 维持 v1.93 比值兜底口径

# 启动飞牌退让门控（2026-09-13 简化，用户定调；2026-09-20 稳成线口径与阈值调整）：仅两道闸——
#   稳成线（FINESSE_NEC_MAKE_HIGH=0.85，**引擎 top1 候选**做成率）之上不飞；
#   之下若 A1 契约必要（做成率 < FINESSE_NEC_MAKE）且 飞牌/榜首
#   ≥ FINESSE_NEC_MIN_RATIO 才必飞，否则比值兜底（≥ FINESSE_NEC_RATIO）才飞；
#   均不满足则退让（尊重引擎）。
#   A2 盈余、B 失败安全、C 升级价值判据已于 2026-09-13 删除。
FINESSE_NEC_MAKE = 0.50              # 判据A1：榜首做成率 < 此值 → 契约本就需要飞牌 → 启动
FINESSE_NEC_MIN_RATIO = 0.50        # 判据A1 下限：飞牌/榜首 ≥ 此值才强制启动——堵住
                                     # "25% 换 45%"的缺口（榜首虽差，飞牌更不能差到离谱）
FINESSE_NEC_MAKE_HIGH = 1.00         # 稳成线（介入·段1）：**引擎 top1 候选**做成率 ≥ 此值
                                     # → 介入闭嘴（无损清将/飞牌全退让，尊重引擎）。
                                     # 2026-09-20 定调口径=引擎 top1 候选做成率（对齐 _top1_make）；
                                     # 2026-09-22 阈值 0.85 → 1.00（用户改 100%）：只有板上钉钉
                                     #   才让介入闭嘴，85~100% 区间交无损清将/飞牌精修——
                                     #   例：4♠ 引擎 top1=99.8%<100%，不被挡，能进无损清将。
FINESSE_NEC_RATIO = 0.85             # 比值兜底阈值（0.90→0.70（2026-09-13）→0.85（v2.13））：
                                     # 非稳成时 飞牌候选 / 榜首 ≥ 此值才放行。v2.13（2026-09-24）
                                     # 门控分子由"押注方向半桶做成率"改为**动作牌全样本做成率**
                                     # （用户定调：决定"要不要飞"应看全局期望而非押对方向的半桶），
                                     # 分子整体下移，阈值相应回升收紧。与 αμ 共用的 FINESSE_RATIO
                                     # 独立——兜底是"差距不大才飞"的口径。

# 首攻与信号方案："standard"（标准方案，源自新睿自然）| 预留扩展（如"reverse"反式信号）
LEAD_SIGNAL_SCHEME = "standard"

# DD 残局精确枚举
DD_ENDGAME_CARD_THRESHOLD = 4    # 每手剩余牌数≤此值时触发枚举所有分布
DD_ENDGAME_MAX_ENUMERATIONS = 5000  # 枚举总数超过此值时回退采样

# LLM 审查已随 dd-αμ-LLM 引擎下线（2026-09-12）：DD_ALPHAMU_SWITCH_CARDS / ALPHAMU_LLM_GAP_CAP 移除

# 引擎粒子数/采样数范围（供 API 配置端点校验用）
DD_PARTICLES_MIN = 100
DD_PARTICLES_MAX = 2000
ALPHA_MU_WORLDS_MIN = 10
ALPHA_MU_WORLDS_MAX = 100

# 防守信号参数
SIGNAL_WEIGHT = 1.3      # 信号一致时权重乘数
SIGNAL_PENALTY = 0.7     # 信号不一致时权重乘数
SIGNAL_MIN_RANK = 8      # ≥此值（8=8）视为高牌信号（欢迎）

# αμ 搜索参数（残局多步前瞻，解决 strategy fusion）
ALPHA_MU_ENABLE = True            # 是否启用 αμ 搜索
ALPHA_MU_ENDGAME_CARDS = 8        # 每手剩余牌数≤此值时启用 αμ（残局）
ALPHA_MU_NUM_WORLDS = 20          # possible worlds 数量
ALPHA_MU_M = 2                    # 论文 M 参数：Max 递归层数（M=1 退化为 PIMC，Min 不减 M）
ALPHA_MU_M_MIN = 1                # αμ 层数 M 下限（设置面板可调）
ALPHA_MU_M_MAX = 3                # αμ 层数 M 上限（M≥2 为 αμ 多步前瞻，M 越大越慢）
ALPHA_MU_MAX_DEPTH = ALPHA_MU_M   # 兼容旧引用
ALPHA_MU_TIME_LIMIT = 60.0        # 时间限制（秒）
