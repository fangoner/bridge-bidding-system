...
FINESSE_NEC_MAKE_HIGH = 0.85         # 稳成线：**引擎 top1 候选**做成率 ≥ 此值 → 退让给引擎，
                                     # 全部飞牌判据冻结（不介入，9砸 分支同口径）；
                                     # 比值兜底同样不参与（防止高成约路线被低成约飞牌替换）。
                                     # 2026-09-20 用户定调：① 口径由"全体候选最高做成率"
                                     # （v1.84 FIX-7）改为"引擎 top1 做成率"，使实现与
                                     # _top1_make 语义一致、且对齐引擎实际要走的路线；
                                     # ② 阈值 0.95 → 0.85（更早退让）。
