...
FINESSE_PROBE_DELTA = 0.10           # 飞牌后果敏感性探针阈值（v1.92 口径）：缺失大牌在
                                     ...
                                     # 进池门票，方向另存探针"方向"键）
FINESSE_COMMIT_DIE_PCT = 0.05         # v1.96 三层接应判据·下界：登记方向桶内
                                     # 引擎最优替代牌做成率 ≤ 此值 → 定约不飞
                                     # 即死，强制接应（6NT 型：西桶 ♦A=0）
FINESSE_COMMIT_ALIVE_PCT = 0.40      # v1.96 三层接应判据·上界：桶内引擎最优
                                     # 替代牌做成率 ≥ 此值 → 定约不依赖这墩飞牌，
                                     # 退让引擎（B26 型：西桶 ♦A=0.542）；
                                     # 介于两界之间/桶数据缺失 → 维持 v1.93 口径
...
FINESSE_NEC_RATIO = 0.70             # 比值兜底阈值（0.90→0.70，2026-09-13 用户调低）：
                                     # 非稳成时 飞牌候选 / 榜首 ≥ 此值才放行。与 αμ 共用
                                     # 的 FINESSE_RATIO 独立——兜底是"差距不大才飞"的宽口径，
                                     # 测试见 0.762（差 24 点）不应放行。
