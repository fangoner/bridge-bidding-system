...
# 启动飞牌退让门控（2026-09-13 简化，用户定调）：仅两道闸——
#   稳成线（FINESSE_NEC_MAKE_HIGH）之上不飞；之下若 A1 契约必要（做成率
#   < FINESSE_NEC_MAKE）且 飞牌/榜首 ≥ FINESSE_NEC_MIN_RATIO 才必飞，否则
#   比值兜底（≥ FINESSE_NEC_RATIO）才飞；均不满足则退让（尊重引擎）。
...
FINESSE_NEC_MAKE_HIGH = 0.95         # 稳成线：做成率 ≥ 此值 → 不飞也成，全部判据冻结（不启动）；
                                     # 比值兜底同样不参与（防止 100% 稳成被 76% 的飞牌替换）
