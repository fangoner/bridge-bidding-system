"""标准自然叫牌体系（JF实战）硬编码约束库。已弃用（deprecated，v1.79 标注）。

⛔ 本文件保留作为研究资产，不再被系统接入参照：
- v1.66 起约束生成唯一入口改到"确认定约与首攻"弹窗（LLM 转换 / 前端确认），
  规则库接入已移除，主代码不再 import 本文件；
- 其产生的旧来源名（hard_coded*/convention_*/cue_bid/overcall_*/unusual_nt 等）
  已被 v1.79 删除的"来源分级"体系覆盖，勿再依赖；
- 文件保留仅供阅读/研究，勿在其基础上继续扩展。
