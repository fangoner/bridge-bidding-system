...
root[first_keyword] = {}
root[first_keyword][second_keyword] = {}

stack = [(-1, root), (0, root[first_keyword][second_keyword])]

print(f'初始栈: {[(d, list(n.keys())) for d, n in stack]}')
print()
...
for line_idx, line in enumerate(lines[start_line:], start=start_line):
    ...
    print(f'  栈顶depth={stack[-1][0]}, 当前depth={depth}')
    ...
    parent = stack[-1][1]
    print(f'  父节点: {list(parent.keys()) if parent else "None"}')
    parent[bid] = node
    print(f'  添加节点: {bid}')
    print(f'  栈: {[(d, list(n.keys())) for d, n in stack]}')
