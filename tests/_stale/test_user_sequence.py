...
print(f"=== 双人模式序列测试（用户给出的序列） ===")
print(f"完整序列: {bidding_sequence}")

# 提取关键词
keyword = extract_retrieval_keyword(bidding_sequence, "自然", "西")
print(f"关键词: {keyword}")
...
for seg in segments:
    ...
    if keyword in content:
        ...
        result = preprocess_jf_content(content, bidding_sequence, "西", keyword)
...
print(f"\n=== 序列解析检查 ===")
print(f"当前叫牌人: 西")
print(f"完整序列: 1C-pass-1S-pass-1NT-")
print(f"西家需要叫牌，前一个叫品: 1NT (南)")
...
for test_keyword in test_keywords:
    ...
    for seg in segments:
        ...
        if test_keyword in content:
            ...
            result = preprocess_jf_content(content, bidding_sequence, "西", test_keyword)
